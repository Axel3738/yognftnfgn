...
export function nastaStegText(r) {
  ...
  switch (r.typ) {
    ...
    // Boten lovade att lagret får adressen — det är VA:n som ändrar den (SOP "Order placed with incorrect delivery address").
    case 'adress': return en ? 'Change the address on the order today, before it ships, per the SOP "Order placed with incorrect delivery address". Then confirm to the customer in one line.' : 'Ändra adressen på ordern i dag, innan den skickas, enligt SOP:en "Order placed with incorrect delivery address". Bekräfta sedan för kunden med en rad.';
    default:
