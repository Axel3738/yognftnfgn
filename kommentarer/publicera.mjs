...
const redan = new Set(existsSync(LOGG) ? readFileSync(LOGG, 'utf8').split('\n').filter(Boolean).map((l) => JSON.parse(l).id) : []);
...
async function tokenFor(sida) {
  if (!sidtoken.has(sida)) sidtoken.set(sida, (await anrop(`${sida}?fields=access_token`, token)).access_token);
...
for (const { id, svar: text } of svar) {
  ...
  try {
    ...
    if (!skarpt) { console.log(`(torrt) ${r.verksamhet}/${r.kanal} → ${kort}`); continue; }
    const ut = ig
      ? await anrop(`${mal}/replies`, tok, 'POST', { message: text })
      : await anrop(`${mal}/comments`, tok, 'POST', { message: text });
    appendFileSync(LOGG, JSON.stringify({ id, svar_id: ut.id, sida: r.sida, verksamhet: r.verksamhet, kanal: r.kanal, text, tid: new Date().toISOString() }) + '\n');
    console.log(`✅ ${r.verksamhet}/${r.kanal} → ${kort} (${ut.id})`);
    postade++;
