...
const en = {
  ...
  costs: {
    smart: {
      ...
      pickHint: "Tap the one that matches. Nothing is saved until you do.",
      pickQuestion: "Which one is right?",
    ...
    market: {
      ...
      help: "Standard applies to every country. Pick a country to set a different cost for it.",
      addLabel: "Country code",
      add: "Add",
      addToggle: "Add a country…",
      tableNote: "Bold = its own cost for that market. Grey with * = uses the standard cost. Pick a market above to edit it. A variant counts as covered when it has a standard cost, or its own cost on every market you sell to.",
      standardCol: "Standard (fallback)",
      perMarketBadge: "per market",
      removeMarket: (name: string) => `Remove ${name} (all its costs)`,
      noOrders: "No orders from this country in the last 90 days.",
      inheritedPlaceholder: (v: string) => `${v} (standard)`,
      quickBody: (name: string) => `Type the cost for ${name}. Leave a field empty to keep the standard cost — it's shown as a suggestion.`,
      activeNote: (name: string) => `Everything you save on this page is the cost for ${name}. Shopify keeps the standard cost.`,
    ...
    tpl6: "# Pack prices: unit cost first, then quantity:total separated by | — e.g. 88.34|2:134.22|50:2900.00 (the total for that quantity, not per unit).",
    ...
    ai: {
      title: "Let AI read your old app",
      body: "Take a screenshot of the cost table in Juicy (or any other app or spreadsheet) and drop it here. AI reads it, matches your products and writes the costs to Shopify. Nothing is guessed — rows it cannot match are listed.",
      drop: "Drop screenshot(s) here",
      dropHint: "PNG or JPG, one or several",
      pasteLabel: "…or paste the table as text",
      run: "Read and write to Shopify",
      reading: "Reading…",
      result: (applied: number, unmatched: number) => `${applied} variants written · ${unmatched} rows could not be matched`,
    ...
    quote: {
      ...
      body: "Got a quote or price list from your supplier? Drop a photo or screenshot here. AI reads the prices, you pick which product each row belongs to, and the cost is saved to Shopify. Foreign currency is converted at today's ECB rate.",
      drop: "Drop quote here",
      dropHint: "Photo, screenshot or PDF page as image",
      pasteLabel: "…or paste the quote as text",
      run: "Read the quote",
      currencyLabel: "Currency in the quote",
      detectedUnknown: (raw: string) => `The quote shows "${raw}", which is not a currency the app knows. Pick the right one.`,
      rowCurrency: (c: string) => `This row is priced in ${c}.`,
      ...
      apply: "Add",
      applied: "Saved",
      converted: (from: string, to: string, rate: number) => `${from} → ${to} at ${rate.toFixed(4)} (ECB today)`,
      noRate: (from: string) => `Could not fetch the ${from} rate — type the cost by hand.`,
      moq: (n: number) => `MOQ ${n}`,
  ...
  settings: {
    ...
    claude: {
      ...
      saving: "Checking…",
      remove: "Disconnect",
      connected: (last4: string) => `Connected — key ending in ${last4}`,
      since: (date: string) => `since ${date}`,
      onServer: "Running on the app's own key. Connect your own to use your account instead.",
      missing: "Claude is not connected. Open Settings and connect it.",
      badKey: "That doesn't look like a Claude key. It starts with sk-ant-.",
      rejected: (reason: string) => `Claude did not accept the key: ${reason}`,
      saved: "Claude is connected.",
    ...
    metaVideo: "Example: how to connect Meta (video)",
    metaHideVideo: "Hide video",
    ...
    accountSelectHelp: "The accounts your Facebook login can see. Pick one to add it — saved as soon as you choose.",
    accountSaved: (name: string) => `Saved — ad spend is now read from ${name}.`,
    accountsTitle: "Connected ad accounts",
    accountsSummed: "Ad spend is the total of every account below.",
    accountAdded: (name: string) => `Added — ad spend from ${name} now counts.`,
    accountRemoved: "Removed. Ad spend from that account no longer counts.",
    removeAccount: "Remove",
    addAccountButton: "Add",
    addAnotherAccount: "+ Add another ad account",
    ...
    campaigns: {
      ...
      include: "Only the campaigns I pick",
      exclude: "Everything except the campaigns I pick",
      modeLabel: "Include or exclude",
      open: "Choose campaigns",
      close: "Close",
      loading: "Fetching campaigns…",
      empty: "No campaigns found in this ad account.",
      spend30: (amount: string, currency: string) => `${amount} ${currency} last 30 days`,
      noSpend: "No spend last 30 days",
      save: "Save selection",
      saved: "Saved — ad spend is being recalculated.",
      pickOne: "Pick at least one campaign.",
      summaryAll: "All campaigns in the account count towards your ad spend.",
      summaryInclude: (n: number) => `Only ${n} campaign${n === 1 ? "" : "s"} count towards your ad spend.`,
      summaryExclude: (n: number) => `${n} campaign${n === 1 ? "" : "s"} are left out of your ad spend.`,
      failed: "Could not fetch the campaigns just now. Try again.",
    ...
    googleService: (namn: string) => `this page is served by the service ${namn}`,
...
const sv: Texts = {
  ...
  costs: {
    smart: {
      ...
      pickHint: "Tryck på den som stämmer. Inget sparas förrän du gör det.",
      pickQuestion: "Vilken är rätt?",
    ...
    market: {
      ...
      standardShort: "Standard",
      help: "Standard gäller alla länder. Välj ett land för att sätta en annan kostnad där.",
      addLabel: "Landskod",
      add: "Lägg till",
      addToggle: "Lägg till ett land…",
      tableNote: "Fet = egen kostnad för marknaden. Grå med * = använder standardkostnaden. Välj marknad ovan för att ändra. En variant räknas som täckt när den har standardkostnad, eller egen kostnad på varje marknad du säljer till.",
      standardCol: "Standard (reserv)",
      perMarketBadge: "per marknad",
      removeMarket: (name: string) => `Ta bort ${name} (alla dess kostnader)`,
      noOrders: "Inga ordrar från det här landet de senaste 90 dagarna.",
      inheritedPlaceholder: (v: string) => `${v} (standard)`,
      quickBody: (name: string) => `Skriv kostnaden för ${name}. Lämna fältet tomt så gäller standardkostnaden — den visas som förslag.`,
      activeNote: (name: string) => `Allt du sparar på sidan blir kostnaden för ${name}. Shopify behåller standardkostnaden.`,
    ...
    tpl6: "# Packpriser: styckpriset först, sedan antal:totalpris med | emellan — t.ex. 88.34|2:134.22|50:2900.00 (totalt för antalet, inte per styck).",
    ...
    ai: {
      title: "Låt AI läsa av din gamla app",
      body: "Ta en skärmbild av kostnadstabellen i Juicy (eller vilken app eller kalkyl som helst) och släpp den här. AI läser av, matchar dina produkter och skriver kostnaderna till Shopify. Inget gissas — rader som inte kan matchas listas.",
      drop: "Släpp skärmbild(er) här",
      dropHint: "PNG eller JPG, en eller flera",
      pasteLabel: "…eller klistra in tabellen som text",
      run: "Läs av och skriv till Shopify",
      reading: "Läser av…",
      result: (applied: number, unmatched: number) => `${applied} varianter skrivna · ${unmatched} rader gick inte att matcha`,
    ...
    quote: {
      ...
      body: "Har du fått en offert eller prislista från leverantören? Släpp en bild eller skärmbild här. AI läser priserna, du väljer vilken produkt varje rad hör till, och kostnaden sparas i Shopify. Utländsk valuta räknas om med dagens ECB-kurs.",
      drop: "Släpp offerten här",
      ...
      detectedUnknown: (raw: string) => `Offerten visar "${raw}", som appen inte känner igen som en valuta. Välj rätt valuta.`,
      rowCurrency: (c: string) => `Den här raden är angiven i ${c}.`,
      ...
      apply: "Lägg in",
      applied: "Sparat",
      converted: (from: string, to: string, rate: number) => `${from} → ${to} till ${rate.toFixed(4)} (ECB i dag)`,
      noRate: (from: string) => `Kunde inte hämta kursen för ${from} — skriv kostnaden för hand.`,
      moq: (n: number) => `MOQ ${n}`,
  ...
  settings: {
    ...
    claude: {
      ...
      saving: "Kontrollerar…",
      remove: "Koppla bort",
      connected: (last4: string) => `Kopplad — nyckel som slutar på ${last4}`,
      since: (date: string) => `sedan ${date}`,
      onServer: "Kör på appens egen nyckel. Koppla din egen för att använda ditt konto i stället.",
      missing: "Claude är inte kopplad. Öppna Inställningar och koppla den.",
      badKey: "Det ser inte ut som en Claude-nyckel. Den börjar med sk-ant-.",
      rejected: (reason: string) => `Claude godkände inte nyckeln: ${reason}`,
      saved: "Claude är kopplad.",
    ...
    metaVideo: "Exempel: så kopplar du Meta (video)",
    metaHideVideo: "Dölj videon",
    ...
    accountSelectHelp: "Kontona din Facebook-inloggning kan se. Välj ett för att lägga till det — sparas så fort du valt.",
    accountSaved: (name: string) => `Sparat — annonskostnaden läses nu från ${name}.`,
    accountsTitle: "Kopplade annonskonton",
    accountsSummed: "Annonskostnaden är summan av alla konton nedan.",
    accountAdded: (name: string) => `Tillagt — annonskostnaden från ${name} räknas nu med.`,
    accountRemoved: "Borttaget. Annonskostnaden från det kontot räknas inte längre.",
    removeAccount: "Ta bort",
    addAccountButton: "Lägg till",
    addAnotherAccount: "+ Lägg till ett annonskonto till",
    ...
    campaigns: {
      ...
      include: "Bara kampanjerna jag väljer",
      exclude: "Alla utom kampanjerna jag väljer",
      modeLabel: "Inkludera eller exkludera",
      open: "Välj kampanjer",
      close: "Stäng",
      loading: "Hämtar kampanjer…",
      empty: "Inga kampanjer hittades i annonskontot.",
      spend30: (amount: string, currency: string) => `${amount} ${currency} senaste 30 dagarna`,
      noSpend: "Ingen kostnad senaste 30 dagarna",
      save: "Spara valet",
      saved: "Sparat — annonskostnaden räknas om.",
      pickOne: "Välj minst en kampanj.",
      summaryAll: "Alla kampanjer i kontot räknas in i annonskostnaden.",
      summaryInclude: (n: number) => `Bara ${n} kampanj${n === 1 ? "" : "er"} räknas in i annonskostnaden.`,
      summaryExclude: (n: number) => `${n} kampanj${n === 1 ? "" : "er"} hålls utanför annonskostnaden.`,
      failed: "Kunde inte hämta kampanjerna just nu. Försök igen.",
    ...
    googleService: (namn: string) => `den här sidan körs av tjänsten ${namn}`,
