...
| Taköverdraget FI: alla 19 videoannonser i SE-kampanjen `120250147343350291` (Meta Graph API) + 15 bildannonser | fi / Finnish | — **inget HeyGen** (Axels beslut 2026-09-18): ElevenLabs STT `scribe_v1` → sonnet-manus → **TTS `eleven_v3`, rösten Martti** (samma i alla 19), ljudspåret bytt | ✅ alla 19 lokaliserade av sonnet-subagent, granskade med skript (priser 126,90/165,90/39 €, inga recensioner, inget "koskaan", inga hastighetslöften) | ✅ finska captions exakt där de svenska ordcaption-pillren satt (`pipeline/no-precis.py`), prisgrafik/slutkort som PNG-lager (`temu/takoverdrag/fi-kampanj/lager-fi.py`) | ✅ uppladdade PAUSED i Magiborsten FI `act_1619718346388201` — se `temu/takoverdrag/FI-KAMPANJ-LOGG.md`. A/B-test Bäver vs CaraShell (Axels beslut 2026-09-18): samma 34 annonser en gång till som **CaraShell-armen** i samma konto (CaraShell-sidan + -pixeln, carashell.se/fi, neutrala slutkort utan logga) — `FI-KAMPANJ-LOGG-CARASHELL.md`; kräver att CaraShell-pixeln är delad till kontot | Motorn: `temu/takoverdrag/fi-kampanj/fi-video.mjs`. eleven_v3 lägger 0,5–1 s tystnad efter varje mening ⇒ pauser kortas/förlängs så VO:n spänner över SE-talets tid (tempo max 1,02). Källornas tysta musikbädd (mono, ~20 dB under talet) går inte att separera — FI-versionerna har bara VO. `rostkoll.py` grönt på alla; ingen har lyssnat |
