# Mastern_GT_314_1 — Three siblings, one gift — 333 kr each

**VARIABELTAGGAR:** vinkel=`present / fars dag — syskonen går ihop` · hook-typ=`påstående (tre syskon, en present, tredjedelen)` · format=`statisk, produktfoto + tre rader` · proof=`räkningen 999 ÷ 3` · offer-i-creativen=`999 kr open, the split as arithmetic, no percentage` · visuell stil=`Mastern-lådan med tre händer på / tre namnlappar` · textmängd=`rubrik + underrad + prisrad + sidfot` · talare=`ingen` · copy_model=`sonnet` · typ=`S` · koncept=`fd-syskonen` · parent=`—` · iteration=`—` · kalla=`swipe` · avatar=`syskonen` · awareness=`product` · begar=`spara-pengar` · mekanism=`244-kronorssnittet: 999 kr blir rimligt delat på tre — prisankaret från givarens sida` · tro=`en present tillsammans slår tre små var för sig` · urgency=`sasong` · hook-mekanik=`none` · confidence=`medium` · lardom=`L-B020`
...
**Why:** Svensk Handel's 244 kr average means the 999 kr price needs a frame for adult children; "we went in together" is how siblings already buy big gifts, and B020 proved an angle static with the open price beats discount statics. Judged on purchases and profit contribution, never on ROAS or CTR alone.
**Deliver as:** `Mastern_GT_314_1` (4:5) and the same file in 1:1.
...
| Photo | Real photo of the Mastern box on a kitchen table with three hand-written gift tags tied to it. Never generated. Product photos from Drive (14xo1cnBbNMhv-i0d7WUbtgfMXVVcvlWf) if the box shot is not possible — then the tags are composed in PIL as flat paper labels. |
...
- No people, no hands in the 1:1 crop; hands are allowed in 4:5 only as three hands on the box edge.
- Calm, no red, no badge.
**Reference:** B020 (angle static, open price, ROAS 2,53, lowest CTR — `docs/mastern-static-analys.md`) — replicate the "one angle, open price" formula; do not replicate B66's discount area.
...
- **"333 kr var" is arithmetic on the live price (999 ÷ 3), not a second price.** It is the only extra kronor amount allowed in this brief; never round it, never write "ca".
...
Whether the pooled-price frame unlocks the adult-child audience at 999 kr.
