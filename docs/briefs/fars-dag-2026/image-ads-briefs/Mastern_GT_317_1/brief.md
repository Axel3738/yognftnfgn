# Mastern_GT_317_1 — Order by the 19th of October — the honest deadline static

**VARIABELTAGGAR:** vinkel=`present / fars dag — sista beställningsdag` · hook-typ=`påstående (datum) — ärlig brådska ur leveranstiden` · format=`statisk produkt + datum` · proof=`butikens fraktlöfte (fem till nio arbetsdagar)` · offer-i-creativen=`999 kr small, no percentage` · visuell stil=`Mastern stor, datumet litet, november-ljus` · textmängd=`rubrik + datumrad + prisrad + sidfot` · talare=`ingen` · copy_model=`sonnet` · typ=`S` · koncept=`fd-sista-dagen` · parent=`—` · iteration=`—` · kalla=`egen-data` · avatar=`partner-maria` · awareness=`promo` · begar=`slippa-krangel` · mekanism=`den enda konkreta brådskan som är sann: leveranstiden — datumet litet, produkten stor (L-B066)` · tro=`beställer jag nu finns den på bordet den åttonde` · urgency=`sasong` · hook-mekanik=`none` · confidence=`medium` · lardom=`L-B066`
...
**Why:** Five to nine business days means the 26th of October is the last day the store's own promise reaches the 6th of November, and the measured p90 says the 19th is the safe date; 277 proved holiday + concrete reason gets delivery, B66 proved a shouting offer eats ROAS. Judged on purchases and profit contribution, never on ROAS or CTR alone.
**Deliver as:** `Mastern_GT_317_1` (4:5) and the same file in 1:1.
...
| Photo | Real product photo (Drive 14xo1cnBbNMhv-i0d7WUbtgfMXVVcvlWf or CDN https://cdn.shopify.com/s/files/1/0947/0174/8548/files/Namnlosdesign-2026-07-06T113034.467.png), Mastern large on charcoal with a hint of November window light composed behind (generated background allowed, product never generated). |
...
- No clock, no hourglass, no red.
- Two versions of the file: "19 oktober" (default) and "26 oktober" (only on Axel's confirmation).
**Reference:** B71 (handwritten "vi beställde för många" tag, ROAS 2,64–2,86) for a concrete reason in picture; B66 (ROAS 1,74) for what not to do.
...
- **The date is confirmed by Axel before production.** Default "19 oktober" (measured p90 ≈ 20 days, `docs/os/EPOST-STRATEGI.md`); "26 oktober" only if Grillkliniken holds the nine business days on the product page. This is the ONLY brief in the batch allowed to promise a delivery date. Pause on the date it names + 1 day.
- **Landing page:** https://grillkliniken.se/products/elektrisk-grillborste

## 10. Primary KPI
**Purchases and profit contribution** `(768 − CPA) × purchases` — break-even CPA 768 kr, break-even ROAS 1,30; kill only against break-even, never against target. No verdict under 300 kr / 3 purchases. Low CTR is not a problem (L-B020: the best angle static had the lowest CTR).

## 11. What we learn regardless of outcome
Whether an honest deadline converts a giver without the discount penalty — and how much of the batch's volume lands in the last week before the date.
