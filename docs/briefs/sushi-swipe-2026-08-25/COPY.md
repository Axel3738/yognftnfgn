# Sushi-Strumporna — copy per annons (swipe av svenskhusman_socks, 2026-08-25)

> ⚠️ **DELVIS FÖRÅLDRAD 2026-08-27.** 3-par-varianten (369 kr) är slutsåld på matstrumpor.se,
> och erbjudandet är nu **Köp 1 – Få 1 GRATIS** (2 lådor, 399 kr). **Copy 5 nedan får inte
> köras** — den bygger helt på prisdifferensen 369→399 kr. Ersättare finns som copy E i
> `docs/briefs/sushi-bogo-2026-08-27/COPY.md`. Copy 1–4 fungerar fortfarande, men lägg till
> BOGO-erbjudandet i primärtexten innan de launchas.

Fem copy-set. Varje set listar vilka bilder i mappen det ska paras med.
A-bilder = riktiga produktfoton · B-bilder = AI-genererade (kie.ai, produktfoto som referens).
Priser: 3 par 369 kr / 5 par 399 kr · matstrumpor.se

---

## Copy 1 — Sushiboxen (kollektionsstämning)
**Bilder:** `A1-sushiboxen-stamning.jpg` · `B4-presentogonblick.jpg` · `B5-flatlay-rosa.jpg`

**Primärtext:**
Öppna locket och det ser ut som en sushibricka — rullade bitar i rad, olika färger, prydligt i varsitt fack.

Titta en gång till. Det är strumpor. Mottagaren ser det på tredje sekunden, inte på första — och den som gav bort den sekunden blir ihågkommen.

**Rubrik:** Sushiboxen som inte är sushi
**Länkbeskrivning:** Strumpor rullade som sushi. matstrumpor.se

**Hook-varianter (alternativ första rad, för test):**
1. Det ser ut som en sushibricka. Det är strumpor.
2. Locket går upp. Ansiktet gör resten.

---

## Copy 2 — Nigiribiten (produktnära)
**Bilder:** `A2-narbild-bitar.jpg` · `B3-fotter-gul.jpg`

**Primärtext:**
En bit ur boxen: hårt rullad, ligger i sitt eget fack, formad precis som en nigiribit.

Rullar du ut den är det ett par strumpor du faktiskt kommer ha på dig i morgon. Ärligt talat — strumpor tar man alltid emot.

**Rubrik:** En nigiribit. Fast strumpa.
**Länkbeskrivning:** Rullad som sushi, funkar som strumpa.

**Hook-varianter:**
1. Ser ut som nigiri. Känns som ett par strumpor.
2. Den här biten går faktiskt att ha på foten.

---

## Copy 3 — Utforska boxen (kollektionen)
**Bilder:** `A3-hela-kollektionen.jpg` · `B1-fotter-bla.jpg` · `B2-fotter-rosa.jpg`

**Primärtext:**
Utforska hela sushiboxen — färgglada par, olika toner, alla rullade och packade som ett äkta sushi-set.

Sushi-Strumpor: den enda strumpkollektionen som får folk att fråga "är det där sushi?"

**Rubrik:** Utforska sushiboxen
**Länkbeskrivning:** Färgglada strumpor, rullade som sushi.

**Hook-varianter:**
1. Olika färger. Samma reaktion varje gång.
2. Färgerna byter. Tricket byter aldrig.

---

## Copy 4 — Frågehook (presenten)
**Bilder:** `B4-presentogonblick.jpg` · `A4-bambubricka-illusion.jpg`

**Primärtext:**
Vem i din närhet skulle faktiskt tro att det är sushi i paketet?

(Vi har en känsla om vem.)

**Rubrik:** Vem tror att det är sushi?
**Länkbeskrivning:** Öppna, titta, skratta. Sen använder man dem.

**Hook-varianter:**
1. Vem skulle öppna paketet och tro att det var mat?
2. Present till någon som "redan har allt" — den här går man på.

---

## Copy 5 — Bundle-offret (2 par till för 30 kr)
**Bilder:** `A5-kampanjgrafik.jpg` (texten ligger redan i bilden) · funkar även med `A3-hela-kollektionen.jpg`

**Primärtext:**
3 par sushi-strumpor: 369 kr. 5 par: 399 kr.

Skillnaden är 30 kr — typ priset på en kaffe — för två par till.

Perfekt för dig som vill ge bort ett par, testa ett själv och ha tre kvar när nästa present dyker upp.

**Rubrik:** 2 par till för 30 kr
**Länkbeskrivning:** 3 eller 5 par sushi-strumpor. matstrumpor.se

**Hook-varianter:**
1. Tre par eller fem — skillnaden är 30 kr.
2. Fem par kostar nästan lika mycket som tre.

---

## Kom ihåg innan launch

- B1–B3 är AI-genererade och har små designavvikelser (tå-/hälfärger) mot de
  fysiska strumporna — jämför mot produkten, eller kör A-bilderna där exakthet krävs.
- All copy är testad mot tre-frågorstestet i `docs/copy-regler.md` (hel-grön).
- Inga påhittade siffror: enda talen är 369/399/30 kr.
