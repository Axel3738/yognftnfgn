...
A customer writes that they want to send a product back and get their money back, and the product is NOT damaged, faulty or wrong. Damaged / wrong item → use the damaged-or-wrong-item SOP instead; the 14-day rule below does not apply to those cases.
...
| First step | Find the DELIVERY date (not the order date) and check whether the request is within 14 days of it |
...
- Return window: 14 days from the day the customer RECEIVED the item — this IS the statutory right of withdrawal (owner's decision 2026-09-22; the store promises nothing longer anywhere). Shipping takes 5–10 business days, so the delivery date is a materially different date from the order date — always count from delivery.
...
- Exceptions: gift cards, perishables and custom/personalised products. **There is no "sale item" exception** — every product sells at a compare-at price, so that exception would void the return right for the whole range. ⚠️ The right of withdrawal cannot be signed away: a customer who invokes it within 14 days of delivery is entitled to a full refund and must not be pushed through a partial-refund offer.
...
4. Count 14 days from the delivery date. Is today inside the window?
...
1. Send the owner a summary: order ID, product(s), delivery date, days since delivery, reason for return, item condition, in/out of the 14-day window, and whether it is a partial return.
...
1. Draft the decline email with the AI prompt template below, in the customer's language, quoting the reason the owner gave (outside the 14-day window / item used).
...
Swedish: "Hej! Tack för att du hörde av dig. Tyvärr kan vi inte godkänna returen för order [ORDER ID] eftersom [ANLEDNING]. Ångerrätten gäller 14 dagar från leveransdatum för oanvända produkter i originalförpackning. Hör gärna av dig om du har andra frågor."
English meaning: Unfortunately we cannot approve the return for order [ORDER ID] because [REASON]. The right of withdrawal is 14 days from the delivery date for unused products in original packaging. Please contact us with any other questions.
