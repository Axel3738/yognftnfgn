...
| Auto-reply bot | **Live since 2026-09-23** (owner's decision): it reads the mailbox every minute, around the clock, and **sends** its replies itself, signed "Kundtjänst Bäverbutiken". You will see them in Sent with the replied arrow on the customer's email. Never reply a second time to a thread the bot answered unless the customer writes again. Priority folder: VA-PRIO. A wrong reply that went out: put it right with the customer yourself, then report it (see the page **Auto-reply bot**) | Owner (switched live 2026-09-23) |
