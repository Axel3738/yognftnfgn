// Orchestrator: generera en våg voiceovers (mp3) med ElevenLabs.
//   node vo.mjs                  -> genererar alla klipp i vågen (Svensk Martin + eleven_v3)
//   node vo.mjs --dry            -> hoppar över API:et, visar bara text + teckenantal
//   node vo.mjs --wave=01        -> välj våg (default 01)
...
//   node vo.mjs --text="Hej" --name=test  -> engångsklipp utan våg-fil
...
// Ladda klipp: antingen engångs-text eller en våg-fil.
let clips;
if (val('--text')) {
  clips = [{ name: val('--name', 'vo-adhoc'), text: val('--text') }];
} else {
  ({ clips } = await import(`./voiceovers/vo-${waveArg}.mjs`));
}
...
console.log(
  `Voiceover-våg ${waveArg}: ${selected.length} klipp · röst "${DEFAULTS.voiceName}" · ${DEFAULTS.modelId}` +
...
console.log(
  dry ? 'Klart (dry).' : `Klart. ${ok}/${selected.length} → pipeline/output/vo-${waveArg}/`
);
