...
//        [--produktbild=<png>] [--slutkort=<png>] [--captions] [--torr]
//
...
if (process.argv[1]?.endsWith('marknadsvideo.mjs')) {
  ...
  try {
    ...
    const dubbIn = klippVid != null ? join(tmp, 'utan-kort.mp4') : kalla;
    if (klippVid != null) {
      kor(['ffmpeg', '-y', '-hide_banner', '-loglevel', 'error', '-i', kalla, '-t', String(klippVid),
    ...
    if (args.captions && existsSync(nySrt)) {
      ...
      kor(['python3', join(ROT, 'pipeline', 'no-captions.py'), farsk, nySrt, capUt], { tillat: true });
      if (existsSync(capUt)) { farsk = capUt; rapport.steg.push('brände in captions'); }
      else rapport.steg.push('captions MISSLYCKADES — videon levereras utan');
    }
    ...
    console.log(`\n✅ ${basename(ut)} — ${slut.langd.toFixed(2)} s (källan ${k.langd.toFixed(2)} s), ${rapport.steg.join(' · ')}`);
