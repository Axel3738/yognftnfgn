#!/usr/bin/env node
// Monterar brittiska katalogen för beavershop.co.uk:
//   batches/*.json (källdata) + translations/*.uk.json (engelsk copy)
//   -> output/catalog.uk.json  (komplett engelsk katalog, API-vänlig)
...
//   PRIS   : GBP = round(SEK/13) - 0.01, minst 0.99. compareAt släpps om <= pris.
//   SKU    : BEAVER-<KAT>-<NNN>[-<V>]  (NNN = löpnr, V = variantnr)
//            Temu-SKU bevaras i metafältet supplier_sku.
//   HANDLE : slugifierad engelsk titel.
...
// Norge-körningens temamallar, per källhandle (sätts inline i productSet)
...
const gbp = n => {
  ...
  const v = Math.max(0.99, Math.round(parseFloat(n) / 13) - 0.01);
  return v.toFixed(2);
};
...
for (const bf of batches) {
  ...
  let t = { products: [] };
  try { t = JSON.parse(readFileSync(new URL(bf.replace('.json', '.uk.json'), tdir))); }
  ...
  for (const p of b.products) {
    ...
    products.push({ src: p, en: tr.en, flags: tr.flags ?? [] });
  }
}
...
let seq = 0;
const out = [];
for (const { src, en, flags } of products) {
  seq++;
  const cat = src.handle === 'garanti-for-saker-frakt'
    ? 'SERV'
    : CAT_BY_COLLECTION[src.collection] ?? CAT_BY_TYPE[src.productType] ?? 'MISC';
  const skuBase = `BEAVER-${cat}-${String(seq).padStart(3, '0')}`;
  let handle = slug(en.title);
  while (seen.has(handle)) handle += '-2';
  seen.add(handle);

  const trVar = Object.fromEntries((en.variants ?? []).map(v => [v.id, v]));
  const variants = src.variants.map((v, i) => {
    const tv = trVar[v.id];
    const price = gbp(v.price);
    let compareAt = gbp(v.compareAtPrice);
  ...
  out.push({
    ...
    title: en.title,
    descriptionHtml: en.descriptionHtml,
    vendor: 'BeaverShop',
    productType: en.productType,
    tags: en.tags,
    status: 'DRAFT',
    seo: en.seo,
    options: en.options ?? src.options,
    variants,
    images: src.images.map((im, i) => ({ url: im.url, altText: en.imagesAlt?.[i] ?? im.altText })),
    flags,
  });
}
...
writeFileSync(new URL('catalog.uk.json', odir), JSON.stringify({ products: out }, null, 1));
...
report.unshift(
  `# Byggrapport brittisk katalog (${new Date().toISOString().slice(0,10)})`,
