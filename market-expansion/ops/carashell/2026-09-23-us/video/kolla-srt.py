...
inga å/ä/ö, inga svenska-bara påståenden. Timecode-rader undantas från skanningen.
    python3 market-expansion/ops/carashell/2026-09-18-us/kolla-srt.py
"""
import re, sys, os, glob
HÄR = os.path.dirname(os.path.abspath(__file__))
ORIG = os.path.join(HÄR, 'srt-orig'); US = os.path.join(HÄR, 'srt-us')
...
for o in sorted(glob.glob(os.path.join(ORIG, '*.orig.srt'))):
    ...
    for (n1, t1, x1), (n2, t2, x2) in zip(bo, bu):
        ...
        # Längdkontrollen är en proxy för TALAD längd och mäter tecken. Skriver den ena
        # sidan ett tal med siffror och den andra med ord ("1129 kronor." mot "One hundred
        # ninety-nine dollars.") ljuger teckenmåttet — svenskan tar lika lång tid att säga.
        # Mätt 2026-09-20 på PD_107_H1 block 5. En kontroll som inte kan köras rapporteras
        # som HOPPAD med orsak, aldrig som grön och aldrig som röd.
        if x1 and x2 and bool(re.search(r'\d', x1)) != bool(re.search(r'\d', x2)):
            hoppade.append(f'block {n2}: längden inte jämförbar (siffror mot ord) — lyssna på den efter render')
        elif x1 and x2 and (len(x2) > len(x1) * 1.35 or len(x2) < len(x1) * 0.55): problem.append(f'längd block {n2}: {len(x2)} tecken mot {len(x1)} svenska')
    if problem: fel += 1; print(f'✗ {key}: ' + ' | '.join(problem))
    else: print(f'✓ {key}: {len(bu)} block' + (' · HOPPAD: ' + ' | '.join(hoppade) if hoppade else ''))
