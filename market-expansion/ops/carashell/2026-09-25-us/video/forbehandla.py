...
for n, rutor in F.items():
    ...
    for i, r in enumerate(rutor):
        ...
        # boxblur-radien måste rymmas i den MINSTA planet: i yuv420p är krominansen halva
        # bredden och halva höjden, och radien får inte överstiga halva den sidan. En låg
        # ruta (PD_107 142 px, RI_103 146 px, mätt 2026-09-23) sprängde den fasta 40:an med
        # "Failed to evaluate filter params: -22" — ett fel som LÄSER som trasig ffmpeg och
        # bara är en ruta som är för låg. Radien klamras därför till rutan.
        radie = max(4, min(40, (h // 2) // 2 - 1, (w // 2) // 2 - 1))
        fc += f";[{cur}]split[a{i}][b{i}];[b{i}]crop={w}:{h}:{x0}:{y0},boxblur={radie}:3[bl{i}];[a{i}][bl{i}]overlay={x0}:{y0}:enable='between(t,{t0},{t1})'[v{i+1}]"
        cur = f"v{i+1}"
