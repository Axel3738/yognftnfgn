#!/usr/bin/env node
// Batchdrivare för US-bildannonserna, CaraShell takskyddet US-runda 3 2026-09-16.
// Kör tools/ops-till-meta.mjs en bild i taget mot US-kampanjen kön dömde
// (--kampanj ur jobb.json). Utan --skarpt bara torrkörning.
//   node market-expansion/ops/carashell/2026-09-16-us-3/ladda-upp.mjs [--bara <mal_namn>] [--skarpt]
// Skriver resultat-meta.json.
...
for (const r of rader) {
  ...
  resultat[n] = { page_id: r.page_id, koncept: r.koncept, exit: p.status, skarpt, resultat: js, logg: js ? undefined : logg.slice(-10) };
  console.log(`=== ${n} exit ${p.status} → ${js ? JSON.stringify(js) : logg.slice(-6).join(' | ')}`);
}
