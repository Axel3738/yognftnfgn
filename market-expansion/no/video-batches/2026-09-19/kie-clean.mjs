// Rensar svensk text ur de 12 källbilderna (Fas 3.2, /translate-no) med Kie AI
...
const BILDER = {};
for (const [slug, p] of Object.entries(batch)) {
  for (const [name, fid] of Object.entries(p.images || {})) {
    BILDER[`${slug}_${name}`] = fid;
  }
}
...
function lasState() {
  try { return JSON.parse(readFileSync(STATE_FIL, 'utf8')); } catch { return {}; }
}
function skrivState(s) { writeFileSync(STATE_FIL, JSON.stringify(s, null, 2)); }
