// Engångsskript: lägger in de 4 norska bildannonserna för Gjerdestolpebøyle i
// befintliga koncept-adsets. Skrivet i stället för att köra no-image-ads.mjs
// rakt av, eftersom det skriptets adName-regex (/_\d+$/) inte matchar våra
// hook-suffixade videonamn (..._1_H1) och därför felaktigt trodde bildannonsen
// redan fanns. Samma API-anrop som no-image-ads.mjs, adName beräknat manuellt.
...
const PREFIX = 'Gjerdestolpebøyle';
...
if (!campaignId) { console.error(`✗ kampanjen "${cfg.campaignName}" finns inte.`); process.exit(1); }
...
for (const adsetCfg of cfg.adsets) {
  const K = adsetCfg.name.split(' - ').pop();
  ...
  const adName = `${PREFIX}_NO_${K}_2_1`;
