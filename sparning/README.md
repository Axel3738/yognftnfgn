...
| Beverbutikken NO | ✅ (`_NO`) | ✅ sedan 2026-09-20 kväll (Cowork steg 0: version `bever-no-produkter-claude-4`, 13 rättigheter) | **live 2026-09-20 21:50 CEST: https://beverbutikken.no/pages/spor** — 430 ordrar/14 d, 438 paket, 150 registrerade (taket; 288 tas av timrutinen), 145 event, 145 paket på sidan. Timrutin `trig_01JqE4TDfLVwpJEHhECGyQFL` (:32). Mejl + meny: `cowork/beverbutikken.md` steg A–C |
| Bæverbutiken DK | ❌ token-svar `app_not_installed` | appen "DK claudeprodukter" (Client ID `f498ee41…`) har rättigheterna sedan 2026-09-20 (version `dk-claudeprodukter-4`) men **är inte installerad** — Coworks filter stoppade installationsklicket | väntar på Axels klick: Dev Dashboard → DK claudeprodukter → Installera app → baeverbutiken.dk. Sedan `node sparning/kor.mjs --butik baeverbutiken` + rutin på :48 |
