...
| `/briefgranskning <nyckel>` | **Måndag + torsdag 07:00 + plats, en per OPS-produkt (Axels beslut 2026-09-18):** dagen efter varje briefrond läser en creative director ALLA briefer Nattvakten lade i produktens hub i den ronden och dömer dem mot `docs/copy-regler.md` (tre-frågorstestet, butikens namn aldrig i annonsen), `docs/os/ANALYSMETOD.md` (vinstbidrag, aldrig ROAS ensamt), `docs/naming-convention.md`, priset ur butiken (läst live) och `products/<nyckel>/dna.md` (dött koncept? variabeltaggar? en variabel?). Skriver **`products/<nyckel>/feedback.md`** (daterad sektion per rond: bra / missat / **tre regler för nästa rond** — Nattvakten läser den i steg 0), **en engelsk kommentar på varje Notion-rad med ett FEL** (aldrig annars), och Discord `#ads-to-do`. Läs-bara: ingen status ändras, ingen brief skapas, inga påhittade siffror. Verktyget `tools/briefgranskning.mjs` mäter (namn, taggar, pris, butiksnamn, ❌ i tre-frågorstestet, KPI mot break-even, IMAGE PROMPT, COPY CARD, dna.md:s "får inga briefer"); sessionen dömer resten och skriver `dom.json`. Byggs av `/notionscalercs setup <nyckel>` (`BUTIKSRUTINER.briefgranskning`, cron med veckodagar `1,4`). **Torrkörd 2026-09-18 på `carashell/takskyddet` mot riktig Notion:** ronden 2026-09-16 (batch #3, 7 bildbriefer) — 5 av 7 nämner butiken i copyn (`på carashell.se` i primärtexten, "CaraShell" i `PD_7_1`/`SP_7_1`), alla 7 saknar hard rule-raden om butiksnamnet; båda är regeln från 2026-09-18 mätt på briefer skrivna 2026-09-16, annonserna är live och pausas inte |
...
| 16:20 | `20 14 * * *` (CEST) / `20 15 * * *` (CET) + 5 min × plats | **Speglingen, en per speglad produkt** — byggs av `/notionscalercs setup <nyckel>` (idempotent) för poster med `spegling` i register.json. Inskrivna 2026-09-18: **CaraShell Taköverdraget** (`carashell/takskyddet`, plats 5 ⇒ **16:45**, `45 14 * * *` CEST / `45 15 * * *` CET, källhub `7ec270ab-908c-82f6-a2a8-0153159b20fa` "BÄVER For CARL Taköverdraget för Husvagn") och **CaraShell Termoskyddet** (`carashell/termoskyddet`, plats 7 ⇒ **16:55**, `55 14 * * *` CEST / `55 15 * * *` CET, källhub `c5a270ab-908c-83e3-b721-81fde8643080` "BÄVER Termoskyddet för Husbil"). ⚠️ **Rutinerna är INTE byggda ännu** (2026-09-18): `list_triggers` på `subscriptions@stonebite.org` är tom — CaraShells rutiner ligger på det andra kontot, och där ska speglingen byggas (samma konto som butikens övriga rutiner). Skriv in trigger-id här först när `list_triggers` visat det. Efterjustering vid bygget: Taköverdraget hade 18 NO-klara rader i `Translation in review` — `--fran "Translation in review,Approved"` EN gång | `/ops-spegla carashell/takskyddet`, `/ops-spegla carashell/termoskyddet` |
| Måndag + torsdag 07:00 | `0 5 * * 1,4` (CEST) / `0 6 * * 1,4` (CET) + 5 min × plats | **Briefgranskningen, en per OPS-produkt** — byggs av `/notionscalercs setup <nyckel>` (idempotent; `node factory/rutin.mjs --tider <nyckel>` visar raden med veckodagarna). Dagen efter briefnätterna (sön + ons 00:01): creative director-dom över rondens briefer → `products/<nyckel>/feedback.md` + kommentar på rader med fel + Discord `#ads-to-do`. Tider per plats: plats 0 = 07:00, 5 = 07:25 (`25 5 * * 1,4` CEST / `25 6 * * 1,4` CET), 7 = 07:35. ⚠️ **Inga rutiner byggda ännu** (2026-09-18, kommandot och verktyget mergade till `main` samma dag) — byggs på `claude5@stonebite.org`, samma konto som butikens övriga rutiner. Skriv in trigger-id här först när `list_triggers` visat det. ⚠️ Den enda butiksrutinen med veckodagar i cronen — `skiftaVeckodagar` i `rutin.mjs` flyttar dagarna om en tid någon gång korsar midnatt i UTC | `/briefgranskning <nyckel>` |
...
| Vad Nattvaktens briefer missade — tre regler för nästa rond | `products/<id>/feedback.md` | Varje `/briefgranskning` (måndag + torsdag), läses av Nattvakten i steg 0 |
...
tisdag med deadline på torsdag hinner gå ut innan nästa körning — och en
obesvarad tvist förloras automatiskt. Tvistkollen läser därför **bara** Shopifys
