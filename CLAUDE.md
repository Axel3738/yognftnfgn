...
| `factory/ugc/va-kit/` | **UGC-outreach för VA:n, FRISTÅENDE paket** (Axels beslut 2026-09-24: inte samma repo, hon kör det på eget Claude-konto — claude.ai Project eller Claude Code med `/ugc <produktlänk>`). SOP för varje utfall, villkor (faktura efter godkänd video, råmaterial alltid, inga rättighetsavgifter, 1 500–3 000 kr/video, långsiktighet som förhandlingskort), vetting, Google Sheet-struktur (Creators/Deals/Avatars, profilbild som bild i cellen, Tier A/B för framtida avatarer), mallar på svenska + engelska. Allt på engelska utom mallarna. Ändras villkoren: ändra både `factory/ugc/villkor.md` och kitets `TERMS.md`. **VA:n får PDF:er, aldrig .md** (Axels dom 2026-09-24): `python3 factory/ugc/bygg-va-paket.py <utmapp>` (kräver `pip install markdown` + global Playwright) |
