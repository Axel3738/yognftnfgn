...
| `/skool <klassrums-länk>` | **Transkribera en hel Skool-kurs till ett läsbart dokument** (Axels fråga 2026-09-17: "transkribera hela denna kurs, alla videor och alla avsnitt, i ett stort välstrukturerat dokument"). `skool/hamta.mjs` loggar in med `SKOOL_EMAIL`/`SKOOL_PASSWORD` i Playwrights Chromium, läser kursträdet, modultexterna, videornas **Mux-undertexter** ("English CC") och bilagorna → `skool/output/<grupp>/<kurs>/`; en subagent per video gör undertexten läsbar (stycken + mellanrubriker med [mm:ss], ordagrant, kontrolleras mot källan) → ett Claude Doc i kursens ordning + `TRANSKRIPTION.md` i repot. Whisper bara för videor utan undertext (jämfört 2026-09-17: `medium.en` gav 95,8 % ordlikhet mot CC — inte bättre). Första körningen: **The Celebrity Code – English** (Lodestar), 7 moduler, 4 videor (158 min), 1 PDF-bilaga. `skool/README.md` |
...
| `skool/hamta.mjs` | Hämtar en Skool-kurs (modultexter, Mux-undertexter, bilagor) ur Axels inloggade session — motorn bakom `/skool`. `skool/README.md` |
