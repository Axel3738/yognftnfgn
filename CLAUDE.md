...
| `/autosvar --brand <id> [--torr\|--skarpt] [--discord] [--loop 60]` | **Kundtjänstverktyget som svarar själv (Axels uppdrag 2026-09-21), alla butiker:** brevlådans nya kundmejl → tre hinkar med rena regler — **ENKEL** (var är min order, leveranstid, adressbyte före leverans, öppettider) besvaras med fakta ur Shopify + 17TRACK, **ARG** (frustration, hot om bank/ARN/recension, "aldrig fått", trasig vara, tredje mejlet utan svar) får Axels lugnande rad inom en minut och flyttas till mappen `VA-PRIO`, **SVÅR** (retur, återbetalning, reklamation, tvist, fel vara, allt obelagt) flaggas bara. Skriver via Loopias webbmejl (`kundtjanst/webmail.mjs` steg 7–11: Roundcubes compose/send/mark/move/save-folder, avlästa ur källkoden 2026-09-21 — **inte mätta live**, lösenordet saknades i byggcontainern). Järnregler som tester: ett automatiskt svar per tråd någonsin (Sent + Drafts + loggen `kundtjanst/autosvar/logg/<butik>.jsonl`, maskerad), ett per kund och dygn, aldrig på tvistord/bilagor/autosvar/listmejl/system/egna adresser, aldrig löften (`harForbjudet`), aldrig en annan kunds order, kundens språk (sv/nb/da/fi/en), signatur = butikens supportnamn. `--torr` = utkast i Drafts (standard; skarpt kräver `--skarpt`). Axels ordning: `--torr` på Bäverbutiken tills 20 utkast i rad är rätt → skarpt en dag → alla butiker. Minut-servern (`--loop 60`) är samma kod; timrutinen på claude.ai byggs först när grenen är på `main`. Brandfiler för NO/DK/FI upplagda 2026-09-21 (`kundtjanst/brands/`): Bæverbutiken och Majavakauppa på Loopia, **Beverbutikken på Domeneshop** (MX mätt) — webbmejlen där är oprövad. MCP-verktygen `mail_reply/mail_draft/mail_flag/mail_move` finns i `loopia-mail`. **Första riktiga torrkörningen 2026-09-21 kväll mot Bäverbutikens brevlåda:** steg 7 svarar 302 (rättat, följs), utkast + flagga + flytta fungerar (utkast i `INBOX.Drafts`, VA-mappen `INBOX.VA-PRIO`). 31 mejl lästa: 1 ENKEL-utkast (rätt), 24 flaggade, 6 hoppade. Fyra regelfel hittade i utkasten och rättade samma kväll: Shopifys kontaktformulär ("Nytt kundmeddelande …", kunden i Reply-To) hoppades som systemmejl — nu kundens mejl (`autosvar/kontaktformular.mjs`); svar i trådar VA:n redan besvarat (References från egen domän/vår adress i citatet räknas nu); leveransfönster som passerat och "första skanningen 2–4 dagar" om 26 dagar gamla paket (`staltFakta`: försenat/inga skanningar/oskickad/tvist ⇒ VA:n); "den trasiga varan" om en vara som var för liten (neutralt X). ⚠️ Två sessioner körde torrkörningen samtidigt 23:28 — sex utkast i Drafts, flaggor från båda. En brevlåda, en session |
...
⚠️ Byggt och testat mot en falsk Roundcube (183 + 21 tester) — aldrig kört mot
en riktig brevlåda: `KUNDTJANST_MAIL_PASS_*` fanns inte i byggcontainern
(mätt 2026-09-21 med `mail_brands`: alla åtta brands ❌). Första körningen
ska vara `--brand baverbutiken --torr` i en container som har nyckeln, och
utkasten läses i Drafts innan något skickas.
