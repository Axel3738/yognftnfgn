...
| Måndag 07:00 | `0 5 * * 1` (CEST) / `0 6 * * 1` (CET) | Kundtjänst veckorapport — alla brands, ranking, Discord `#customer-service` per brand. **Inte byggd som Routine än** (2026-09-12): koden, kommandot och `node kundtjanst/setup.mjs` finns. ⚠️ **IMAP går inte från claude.ai** (mätt 2026-09-12: proxyn bryter port 993 mot Loopia, Gmail och Office 365 — bara HTTPS släpps, även på nätverksnivån Full). Rutinen på claude.ai läser därför mejlen via **Gmail-connectorn → jobbfil → `--jobb`** (Loopia vidarebefordrar `hello@<brand>` till Gmail), eller så körs `run.mjs` där nätet är öppet. Byggs med `/rutin /kundtjanst --alla --discord 07:00` (`--dagar 1`) och Gmail kopplad på rutinen | `/kundtjanst --alla --discord` |
...
Klienten säger det själv (`PROXY_SPARRAR_PORTEN`) och hoppar brandet. På
claude.ai läses mejlen i stället ur en **jobbfil** (`--jobb <fil.json>`) som
sessionen bygger med Gmail-connectorn; IMAP direkt fungerar där nätet är öppet
(lokalt, cron, Railway). Bygg aldrig om IMAP-klienten "för att den inte svarar"
— det är nätet.
