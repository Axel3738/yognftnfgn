...
**Produkt:** Övervakningskamera Trådlös – Dubbellins PTZ med AI-Spårning · 799 kr (jämförpris 1 000 kr, spara 201 kr)
...
5. **Recensionerna är importerade, inte organiska — verifierat mot Judge.me
   ...
   organiska recensioner finns (600 recensioner över hela butiken genomsökta,
   plus `product_handle`-filter). **Review-bilder kan inte byggas** förrän en
   riktig kund skrivit något. Behandla inte sidans recensioner som VoC.
6. **CBO-svält, tredje gången bekräftad:** batch #1:s 6 statiska har efter 4 dygn
   3–226 kr spend, batch #1:s 6 videor (launchade 2026-09-05) 2–11 kr efter ett
   dygn, batch #2:s 5 statiska (launchade 09-04/05) 4–13 kr. Ingen ny creative
   får budget bredvid `SP_2`/`CS_3`. Läs inte låg spend som svag creative.
7. **Rättelse av förra körningens fynd:** batch #1:s 6 videobriefer (`SP_4_H1`,
   `SP_5_H1`, `SP_6_H1`, `RI_1_H1`, `CO_1_H1`, `AU_1_H1`) **ÄR levererade och
   launchade** — skapade i kontot 2026-09-05 11:42–12:01, Notion-status
   `Translation in review` (uppladdade av `/notionkorning`, plockade av
   `/oversatt NO`). Förra DNA-versionen skrev "ej levererade" — det gällde
   09-03, inte längre. Batch #2:s 4 videor (`SP_8_H1`, `SP_9_H1`, `SP_10_H1`,
   `AU_2_H1`) står däremot fortfarande som `Draft` i Notion och finns inte i
   kontot.
...
Alla koncept med visningar (G, CS, SP, PD, batch #1-videorna) har hook rate
85–98 %. Skillnaden ligger nedströms. Skriv inte briefer för att "förbättra
hooken".
...
- **Arlo:** "Fler AI-detekteringar för din säkerhet", "12x zoom med
  autospårning", "Enkel & snabb installation", "Stoppa hot innan de kommer nära".
  AI-detektering är alltså en paritetsclaim i kategorin — vårt övertag är att
  **visa** den (röda rutor på riktig film), inte att påstå den.
- **NEXSMART:** säljer 2-pack/4-pack + varningsklistermärke och larmpaket per
  kvadratmeter → bekräftar bundle-idén i backloggen (BLOCKER, Axels beslut).
- **Vyzora:** "Kamera + belysning i ett", "Smart WiFi-kamera med LED".
- **Verisure:** varumärkesannonser utan produkttext.
- **Travella World:** "Alltid full koll på hemmet".
- Våra egna `AU_1_H1` ("Teknikern visar vad du faktiskt får") och `LI_1_1`
  ("5 av 5. Inga kompromisser.") syns i samma sök.
