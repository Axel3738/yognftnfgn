...
**VARIABELTAGGAR:** vinkel=`offer/överlager` · hook-typ=`påstående` · format=`statisk` · proof=`demo` · offer-i-creativen=`ingen offer` · visuell stil=`person i bild` · textmängd=`rubrik+underrad` · talare=`person i bild (stillbild)`
*(Läses av nästa `/cs` för att gruppera vinstbidrag per variabelvärde. Ändra dem inte utan att ändra creativen.)*

**Type:** Static in the offer angle, **person in frame**. Fifth arm of the SO_16 image test.
...
**Hypothesis:** A person in frame outperforms product-only images, because a still of someone using the product carries the same trust signal that makes the video block convert.

**Kept:** The SO copy block, the black cover, the CTA, the landing page, the ad set.

**Changed (isolated variable):** **The image only.** A person is present. Copy, headline, CTA, landing page and ad set are identical to SO_16_1 through SO_16_4.
...
**Ad set:** Motorhölje SO Batch 6 — same ad set as SO_16_1 through SO_16_4
