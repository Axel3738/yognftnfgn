...
**VARIABELTAGGAR:** vinkel=`problem/lösning + kvalificering (storlek)` · hook-typ=`fråga` · format=`statisk` · proof=`demo` · offer-i-creativen=`ingen offer` · visuell stil=`grafik+produkt` · textmängd=`rubrik+lista` · talare=`ingen`
*(Läses av nästa `/cs` för att gruppera vinstbidrag per variabelvärde. Ändra dem inte utan att ändra creativen.)*

**Type:** Static in the product-demo angle. **Size-finder grid.** Fourth attempt at the size-qualification idea, first one on an image.
...
**Hypothesis:** Making the reader find their own hk band before clicking raises purchases per click above the PD block's 2,4 %, by filtering out people whose engine we cannot fit well.

**Kept:** The PD angle, the black cover, the CTA, the landing page.

**Changed (isolated variable):** **The image becomes a size finder** instead of a demo photograph.
...
**Ad set:** Motorhölje PD Batch 6 — same ad set as PD_19_2
