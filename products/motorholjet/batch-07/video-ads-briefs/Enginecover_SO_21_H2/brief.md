# Enginecover_SO_21_H2

**VARIABELTAGGAR:** vinkel=`offer/överlager` · hook-typ=`siffra/pris` · format=`video 20 s, voiceover+broll` · proof=`inget (produktclaim)` · offer-i-creativen=`pris nämns (299 mot 367)` · talare=`röst utan ansikte`
*(Läses av nästa `/cs` för att gruppera vinstbidrag per variabelvärde. Ändra dem inte utan att ändra creativen.)*

**Type:** Video in the offer angle. **Hook variant of `SO_21_H1`** — the opening line is the only difference.
...
**Hypothesis:** A price-forward opening beats the overstock-explanation opening, because the number does the stopping work in a feed.

**Kept:** **Everything from second 4 onward, byte-identical to `SO_21_H1`.**

**Changed (isolated variable):** **The first four seconds only.**
