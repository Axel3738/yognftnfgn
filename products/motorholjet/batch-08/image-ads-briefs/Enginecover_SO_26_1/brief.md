...
**VARIABELTAGGAR:** vinkel=`offer/överlager` · hook-typ=`påstående` · format=`statisk` · proof=`inget (produktclaim)` · offer-i-creativen=`inget pris i bild` · visuell stil=`textfri produktbild + rubrik` · textmängd=`rubrik+underrad` · talare=`ingen` · nedre-tredjedel=`mekanik`
*(Läses av nästa `/cs` för att gruppera vinstbidrag per variabelvärde. Ändra dem inte utan att ändra creativen.)*

**Type:** Static in the offer angle. Lower third carries **a mechanism line**. One of **four** that share one photograph and differ only in that line.
...
**Prisbevis i bild fick äntligen ett läsbart svar, och det var negativt.**
...
**Hypothesis:** Mechanism beats price in the lower third, because the doubt that stops the purchase is about whether the thing works, not about what it costs.

**Kept:** **Fotografiet, exakt som i `SO_16_1`.** Samma exporterade fil, samma beskärning, samma rubrik
`Skydda motorn vid bryggan`, samma copy-block, samma adset.

**Changed (isolated variable):** **The lower third only** — **a mechanism line** where the price block used to be.
...
**Ad set:** Motorhölje SO Batch 8 (ABO) — all four SO_26 variants, equal budget
...
Set the line below in the lower third, clean and typographic. No badge, no starburst, no box.
...
| Nedre tredjedel | Vattenavvisande 420D Oxfordtyg – dragsko håller höljet på plats. | Water-repellent 420D Oxford fabric – a drawstring holds the cover in place. |
...
**Köp per klick, alla fyra mot varandra och mot `SO_16_1`:s 3,3 %.** De **måste** ligga i samma
