...
**VARIABELTAGGAR:** vinkel=`offer/överlager` · hook-typ=`påstående` · format=`statisk` · proof=`inget (produktclaim)` · offer-i-creativen=`inget pris i bild` · visuell stil=`textfri produktbild + rubrik` · textmängd=`rubrik+underrad` · talare=`ingen` · nedre-tredjedel=`storlek`
*(Läses av nästa `/cs` för att gruppera vinstbidrag per variabelvärde. Ändra dem inte utan att ändra creativen.)*

**Type:** Static in the offer angle. Lower third carries **a size line**. One of **four** that share one photograph and differ only in that line.
...
**Hypothesis:** Answering the fit question in the lower third beats price, because size is the one thing a buyer cannot check from the image.
...
**Changed (isolated variable):** **The lower third only** — **a size line** where the price block used to be.
...
| Nedre tredjedel | Finns i storlekar för utombordare 6–250 hk. | Available in sizes for outboard motors 6–250 hp. |
