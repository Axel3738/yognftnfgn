...
**VARIABELTAGGAR:** vinkel=`offer/överlager` · hook-typ=`påstående` · format=`statisk` · proof=`inget (produktclaim)` · offer-i-creativen=`ingen offer` · visuell stil=`textfri produktbild + rubrik` · textmängd=`rubrik` · talare=`ingen` · nedre-tredjedel=`inget`
*(Läses av nästa `/cs` för att gruppera vinstbidrag per variabelvärde. Ändra dem inte utan att ändra creativen.)*

**Type:** Static in the offer angle. Lower third carries **nothing at all**. One of **four** that share one photograph and differ only in that line.
...
**Hypothesis:** An empty lower third beats all three filled versions, because the photograph carries more without a caption competing with it.
...
**Changed (isolated variable):** **The lower third only** — **nothing at all** where the price block used to be.
