...
**VARIABELTAGGAR:** vinkel=`offer/överlager` · hook-typ=`påstående` · format=`statisk` · proof=`inget (produktclaim)` · offer-i-creativen=`inget pris i bild` · visuell stil=`textfri produktbild + rubrik` · textmängd=`rubrik+underrad` · talare=`ingen` · nedre-tredjedel=`garanti`
*(Läses av nästa `/cs` för att gruppera vinstbidrag per variabelvärde. Ändra dem inte utan att ändra creativen.)*

**Type:** Static in the offer angle. Lower third carries **a guarantee line**. One of **four** that share one photograph and differ only in that line.
...
**Hypothesis:** Risk reversal beats price in the lower third, because the hesitation is about fit rather than money.
...
**Changed (isolated variable):** **The lower third only** — **a guarantee line** where the price block used to be.
...
| Nedre tredjedel | 30 dagars nöjd-kund-garanti på ditt motorhölje. | 30-day satisfaction guarantee on your motor cover. |
