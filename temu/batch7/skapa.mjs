...
// Kör: node temu/batch7/skapa.mjs <se|no> [--skarp] [nyckel]
import { Butik } from '../api.mjs';
import { GARANTI4, RUBRIKER4 } from '../utrullning/texter4.mjs';
import { T7_SV } from '../utrullning/texter7.mjs';
...
const BILD = { grindhjul: 'grindhjul', termoskydd: 'termoskydd-matt', solcellslarm: 'solcellslarm',
  stegstod: 'stegstod', infartslarm: 'infartslarm', sittkapp: 'sittkapp' };
const OFFERTNAMN = { grindhjul: 'Fjäderbelastat stödhjul', termoskydd: 'Utvändigt termoskydd', solcellslarm: 'Solcellsdrivet rörelselarm',
  stegstod: 'Väggstöd/stegstöd', infartslarm: 'Trådlöst infartslarm', sittkapp: 'Hopfällbar sittkäpp' };
...
// Norsk copy läses ur texter7-no.mjs när den finns; annars körs bara SE.
let T = T7_SV;
if (land === 'no') {
  const f = path.join(HÄR, '..', 'utrullning', 'texter7-no.mjs');
  if (!existsSync(f)) { console.error('texter7-no.mjs saknas — kör den norska översättningen först'); process.exit(1); }
  T = (await import(f)).T7_NO;
}
...
for (const [nyckel, t] of Object.entries(T)) {
  ...
  const sku = T7_SV[nyckel].sku;          // SKU är gemensam mellan butikerna
  const kategori = T7_SV[nyckel].kategori;
  ...
  if (!skarp) {
    ...
    console.log(`    ${p.pris} / ${p.jamfor} · cogs ${p.cogs} · ${t.bullets.length} bullets · ${T7_SV[nyckel].varden ? T7_SV[nyckel].varden.length + ' varianter' : '1 variant'} · bild ${existsSync(bildfil) ? 'ok' : 'SAKNAS ⚠️'}`);
    continue;
  }
    ...
    await b.mutera(
      ...
      { productId: pid, strategy: 'REMOVE_STANDALONE_VARIANT',
        variants: t.varden.map((namn, i) => ({ ...gemensamt, optionValues: [{ optionName: t.option, name: namn }],
          inventoryItem: { sku: sku + T7_SV[nyckel].suffix[i], tracked: false, cost: String(p.cogs) } })) },
