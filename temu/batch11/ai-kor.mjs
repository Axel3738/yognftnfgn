...
async function video() {
  ...
  for (const [id, a] of Object.entries(AI)) {
    ...
    const r = await (await fetch('https://api.kie.ai/api/v1/veo/generate', { method: 'POST', headers: h, body: JSON.stringify({ prompt: a.video.prompt + GUARD_VIDEO, imageUrls: [ref(id, a.video.ref)], model: 'veo3_fast', aspectRatio: '16:9', generationType: a.video.typ || 'REFERENCE_2_VIDEO', enableFallback: true }) })).json();   // enableFallback: KIE byter modell när Veo svarar 500 "Internal Error" (kamadohuv/värmesulor fem gånger i rad) — bara 16:9/9:16
