// Skapar batch 9 (ark A) och batch 10 (ark B) i bäverbutiken.se och beverbutikken.no.
// Kör:  node temu/batch9/skapa.mjs <se|no> [--skarp] [id]
//
// Underlag:
//   temu/batch9/fakta.mjs     — låsta fakta, SKU och kategori-GID
//   temu/batch9/copy.json     — copy per produkt och språk (Sonnet-skribenter + granskare)
//   temu/batch9/priser.mjs    — prismatrisen, ark A (ARK=a) + ark B (ARK=b)
//   /tmp/b9/ut/<id>/ och /tmp/b9/ut-no/<id>/ — bilderna per språk
//
...
const MAPP = { se: '/tmp/b9/ut', no: '/tmp/b9/ut-no' };
...
if (!['se', 'no'].includes(land)) { console.error('Användning: node temu/batch9/skapa.mjs <se|no> [--skarp] [id]'); process.exit(1); }

const COPY = JSON.parse(readFileSync(path.join(HÄR, 'copy.json'), 'utf8'));
const PRIS = ['a', 'b'].flatMap((ark) => JSON.parse(execFileSync(process.execPath, [path.join(HÄR, 'priser.mjs'), '--json'],
  { encoding: 'utf8', env: { ...process.env, ARK: ark } })));

// Inga produkter i batch 9/10 har varianter (harVarianter = false nedan).
const STORLEKAR = [];
...
for (const [id, f] of Object.entries(FAKTA)) {
  ...
  if (!skarp) {
    ...
    console.log(`    ${p.pris} / ${p.jamfor} · cogs ${p.cogs} · ${t.bullets.length} bullets · 1 variant`);
  ...
  const skapad = await b.mutera(
    ...
    { product: { title: t.titel, vendor: VENDOR[land], status: 'DRAFT', templateSuffix: 'claudeprodukter',
        ...
        ...(harVarianter ? { productOptions: [
          { name: t.optionNamn, values: (t.optionVarden || []).map((n) => ({ name: n })) },
  ...
  const st = await b.mutera(
    ...
    { input: media.map(([roll, ext]) => ({ filename: `b${f.batch}-${id}-${roll}-${land}.${ext}`,
