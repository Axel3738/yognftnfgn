...
| Slå på NOK på Norge-marknaden | **OBEKRÄFTAT** | `marknad.mjs` säger "API-spärrat i unified markets" utan att citera anrop eller felkod — ärvt. Axel klickade det 2026-09-10 (AdventLane, `market.currencySettings.baseCurrency = NOK`); ingen har provat `marketCurrencySettingsUpdate`. Klick i checklistans avsnitt 5 tills dess. |
| Fasta NOK-priser per marknad | **MÄTT: GÅR** | AdventLane 2026-09-10: `catalogCreate` (context marketIds) → `publicationCreate` (autoPublish, ALL_PRODUCTS) → `priceListCreate` (currency NOK, parent adjustment 0 %) → `priceListFixedPricesAdd` (439 / jämför 579). Tillbakaläst med `productVariant.contextualPricing(context:{country: NO})` = 439,00 NOK. Kräver att marknaden redan har NOK som basvaluta (raden ovan). Paketnivåerna (2 stk 746,30 / 3 stk 1 053,60) räknar temat (`ms-paket.js`, 15/20 %) på enhetspriset — inget att sätta. Skriptet: kör som `nok-priser` i sessionen, borde in i `factory/marknad.mjs`. ⚠️ Kundvyn: marknaden utan egen webbnärvaro väljs på besökarens LAND — `curl adventlane.se/nb/…` från containern visar SEK, `…/nb/…?country=NO` visar NOK 439. Verifiera alltid med `?country=NO`. ⚠️ **Uppdatering 2026-09-11:** bara racingkalendern har `originType: FIXED`. De elva nya står som `RELATIVE` — alltså inget satt NOK-pris alls, utan SEK-priset växlat i kassakurs (399 kr → 391 NOK, 449 → 440, 349 → 342). Det är inte trasigt, men det är inte heller ett *valt* pris. Fasta NOK-priser kräver Axels beslut, sedan `priceListFixedPricesAdd` enligt receptet ovan. |
| Lägga produkter i en BEFINTLIG kollektion | **MÄTT: GÅR — men inte i `collectionUpdate`** | 2025-07 svarar `products cannot be specified during update` (AdventLane 2026-09-11, när elva kalendrar skulle in i `kalendrarna`). Medlemskapet skrivs med `collectionAddProducts` EFTER uppdateringen; `products` i input gäller bara `collectionCreate`. Rättat i `shopify.mjs → skrivKollektion`. Innan dess stoppade hela kollektionssteget med noll produkter tillagda. |
| Läsa alla metaobjekt i Liquid (`shop.metaobjects.<typ>.values`) | **MÄTT: TAK 50** | AdventLane 2026-09-11: tolv produkter × sex paketnivåer = 72 metaobjekt, **exakt 50** renderades. Tre produktsidor stod helt utan paketväljare och en fick två nivåer av sex — tyst, utan felmeddelande, och med trippelkollen grön eftersom nivåerna FANNS i butiken. Snippeten läser därför produktens eget metafält `opf.paket_nivaer` (list.metaobject_reference, skrivs av `paket.mjs`) i första hand. Den globala listan är reserv för butiker byggda innan fältet fanns. **Regeln: iterera aldrig hela butikens metaobjekt i en nischbutik.** |
