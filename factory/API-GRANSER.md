...
| Översätta TEMAINSTÄLLNINGAR (`brand_description` m.fl.) på trial | **MÄTT: GÅR INTE på trial** | `translatableResources(resourceType: ONLINE_STORE_THEME_SETTINGS_CATEGORY)` → 0 resurser, `translatableResourcesByIds(OnlineStoreThemeSettingsDataSections/<tema>)` → 0 rader (CaraShell yitrbk-m3, trial, 2026-09-10). TackleBay 2026-09-08 (plan vald) såg typen. **OBEKRÄFTAT efter plan.** Vägen runt är inbyggd: sidfotens brandtext skrivs som text-block i `footer-group.json` (sektionsgrupp, översättbar) i stället för som inställning. |
| Sätta NOK-pris + jämförpris för marknaden Norge | **MÄTT: GÅR** | Tre anrop (CaraShell 2026-09-11): `priceListCreate` (currency NOK, parent adjustment 0 %) → `catalogCreate` (status ACTIVE, `context.marketIds` = Norge, `priceListId`) → `priceListFixedPricesAdd` (price + compareAtPrice i NOK per variant). Förutsätter att NOK redan är marknadens basvaluta (admin-klick). Tillbakaläst som `originType: FIXED`, och som norsk kund på carashell.se: `Shopify.currency = NOK`, 1 106,00 / 1 382,50. ⚠️ Paketnivåerna (ms_paketniva) och rabattkoderna ligger kvar i SEK — NOK-paketnivåer är fortfarande ett eget steg före norska annonser. **Sedan 2026-09-16 är receptet kod: `factory/prislista.mjs` (steg 17b `prislista` i kedjan) — ur `ekonomi.marknadspriser`, idempotent, med tillbakaläsning och basvalutakoll (`market.currencySettings.baseCurrency`) före anropen. **Första körningen mätt 2026-09-16 på CaraShell (`/ny-marknad carashell US`): steget gick igenom, `market.currencySettings.baseCurrency` på den nyskapade marknaden USA svarade `null` ⇒ 🖐 med klicket i klartext, noll prislisteanrop, NOK-raden orörd. Klicket "USD som marknadens valuta" måste alltså göras FÖRE `--igen prislista`, inte efter — API:t ser ingen basvaluta alls på en marknad som just skapats.** |
| Se butiken som AMERIKANSK kund (kundvy i USD) | **MÄTT: GÅR — språk + land; valutan följer admin** | Samma `POST /localization` som för Norge med `country_code=US`, `language_code=en` → 302 till `return_to`, nästa GET `/en/products/<handle>` bär `Shopify.country = "US"`, `lang="en"`, "Add to cart"/"Buy now" och landsväljaren visar `US`. `Shopify.currency` stod kvar på **SEK** (CaraShell 2026-09-16 06:44) så länge USD inte var påslagen som marknadens valuta — /en byter språk, inte valuta. Med USD påslagen (07:05): `{"active":"USD","rate":"0.10444188"}`, fast pris **$199.00** där prislistan finns, Shopifys omräkning **$59.00** (559 kr) där den saknas. ⚠️ Shopify väljer dessutom marknad efter besökarens IP för browser-lika klienter: containern (Ohio) fick `localization=US` på svenska `/` — kundvy-kor.mjs sätter därför landet uttryckligen per vy (`sattLokalisering`). |
| Se butiken som NORSK kund (kundvy i NOK) | **MÄTT: GÅR** | `POST /localization` med `form_type=localization`, `_method=put`, `country_code=NO`, `language_code=nb`, `return_to=…` → 302 + kakan `localization`; nästa GET renderar NOK. Utan `_method=put` svarar Shopify **404** (CaraShell 2026-09-11). Landsväljaren i HTML (`country_code`-värden) visar vilka länder som går att välja. |
...
| Byta PRIMÄRSPRÅK | **OBEKRÄFTAT (API) — MÄNSKLIGT + DYRT** | `shopLocaleEnable`/`shopLocaleUpdate` hanterar extra språk — om primärspråket går att byta via API har ingen provat. I admin går det (Settings → Languages, "Change default"), men Shopifys hjälp (läst 2026-09-16) säger: *"Changing your default language deletes your existing translations for the language that you switch to, and those translations don't become your store's content."* Inget översätts automatiskt, det gamla primärspråket försvinner ur språklistan tills det läggs till igen. För fabriken betyder det: allt underlag om, alla `oversattning-<locale>.json` om, och en butik med annan källspråksordning än de andra. Gör det inte för en app-inställning — Judge.me:s knapptexter löses med "Enable multi-language widgets" (`PROCESS.md` punkt 21). ⚠️ `PROCESS.md` listar primärspråk, valuta, primärmarknad och shop-mejl som "kan INTE sättas via API", men citerar bara ett anrop och en felkod för **butiksnamnet** (REST 406). Resten av den raden är alltså ärvd, inte mätt — därför står de kvar som OBEKRÄFTAT här. |
| Läsa/ändra COOKIE-RUTAN per land | **MÄTT: GÅR (läsning)** | `privacySettings { banner { autoManaged enabled } dataSaleOptOutPage { autoManaged } }` + `consentPolicy { countryCode regionCode consentRequired dataSaleOptOutRequired }` (319 rader) svarade på CaraShell 2026-09-16 — appen bar `read_privacy_settings` + `write_privacy_settings` fast de inte står i `KRAVDA_SCOPES`. Skrivning: `consentPolicyUpdate(consentPolicies: [ConsentPolicyInput])`, inte provad. ⚠️ Rutan följer **besökarens land**, inte marknaden eller domänen: US står `consentRequired: false` på alla 52 rader (ingen ruta för amerikaner, verifierat i headless Chrome från Ohio), SE/NO `true`. "Stäng av den i USA" är därför redan gjort av Shopify. |
| Ge en MARKNAD en EGEN DOMÄN (carashell.com → USA) | **MÄTT: GÅR** | Domänen kopplas i admin (Settings → Domains, Axel 2026-09-16), sedan `webPresenceCreate(input: { domainId, defaultLocale: "en", alternateLocales: [] })` → `marketUpdate(id: USA, input: { webPresencesToAdd: [ny], webPresencesToDelete: [primärdomänens + myshopify-närvaron] })`. Tillbakaläst `markets → USA → webPresences: carashell.com/en`; `carashell.com/products/<handle>` svarar `Shopify.locale en`, `Shopify.country US`, USD. `/en/`-mappen finns INTE på den domänen (404) — länkar byggs utan språkmapp (`opsmarknader.marknadslank`). CaraShell 2026-09-16. |
| Dela en EGEN DOMÄN mellan två marknader | **MÄTT: GÅR INTE** | `marketUpdate(webPresencesToAdd: [<carashell.com-presencen>])` från en annan marknad svarar `RESOURCE_NOT_FOUND` (CaraShell 2026-09-17, GB som DRAFT — raderad). Primärdomänens presence (.se + myshopify) delas däremot fint (Norge har dem). Fler länder på samma domän = fler länder i SAMMA marknad (`lander:` på raden). |
| Lägga FLER LÄNDER i en befintlig marknad | **MÄTT: GÅR** | `marketUpdate(id, input: { conditions: { conditionsToAdd: { regionsCondition: { regions: [{ countryCode }] } } } })` — USA fick GB, CA, AU, NZ 2026-09-17, tillbakaläst i `regions`. Ett land som redan ligger i en annan marknad avvisas. |
| Slå på LOKALA VALUTOR i en marknad | **MÄTT: GÅR — och slår på valutorna i Shopify Payments själv** | `marketUpdate(id, input: { currencySettings: { localCurrencies: true } })` (CaraShell 2026-09-17): `enabledPresentmentCurrencies` gick från NOK,SEK,USD till AUD,CAD,GBP,NOK,NZD,SEK,USD utan ett klick. Basvalutan (USD) sattes däremot i admin dagen innan — `baseCurrency` via API är fortfarande oprovad. Priserna i de lokala valutorna räknas om från prislistans fasta USD-pris (+ ~2–3 %, hela tal). |
| Byta PRIMÄRMARKNAD | **MÄTT: GÅR INTE** | `MarketUpdateInput` (2025-07) har inga `primary`-fält — introspekterat 2026-09-11 på CatCabin: `name, handle, conditions, catalogsToAdd/Delete, webPresencesToAdd/Delete, makeDuplicateUniqueMarketsDraft, status, currencySettings, removeCurrencySettings, priceInclusions`. Primärmarknaden är ett klick i admin (Settings → Markets). ⚠️ Fel primärmarknad är inte kosmetiskt: när Norge blev primär på CatCabin blev VARJE variant `available: false` i kundvyn (paketväljaren och sticky-knappen försvann) — se `PROCESS.md`. |
