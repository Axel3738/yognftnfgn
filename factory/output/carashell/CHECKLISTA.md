# Store Launch Checklist — CaraShell (manual steps)
...
- [ ] Online Store → Themes → **CaraShell – CRO v1** → **Publish**
- [ ] Settings → General → Store name → **CaraShell** → Save
...
- [ ] business.facebook.com → Settings → Pages → Add → Create a new Page: **CaraShell**
...
- [ ] Discord → + → Create server: **CaraShell — OPS**
...
- [ ] Write to Claude Code: **"Store ready: CaraShell"** – it creates the pixel and builds the Discord channels
...
- [ ] WeTracked → paste the **pixel ID**: **28589207184025756**
- [ ] Events Manager → Data sources → **CaraShell** → Settings → Conversions API → **Generate access token** → copy it
...
- [ ] Open **carashell.se** on a phone as a customer: add to cart → checkout
...
- [ ] Write: **/ny-annonser carashell** + the Bäverbutiken product link
- [ ] The link is needed once per store — after that just **/ny-annonser carashell**
...
- [ ] The owner writes **"Launch: CaraShell"** in that session – Claude checks the store is live and the pixel fires, then sets the campaigns ACTIVE
