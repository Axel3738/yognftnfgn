...
- [ ] This store's ID is **kalender**. You need it twice: in the variable names below, and in the build command.
- [ ] Open the Claude session's **Environment** → ADD these 4. Do NOT touch the ones without an ID at the end — those belong to other stores:
  `SHOPIFY_SHOP_KALENDER` = the store's .myshopify.com address
  `SHOPIFY_CLIENT_ID_KALENDER` = the Client ID
  `SHOPIFY_CLIENT_SECRET_KALENDER` = the Client secret
  `SHOPIFY_STOREFRONT_PASSWORD_KALENDER` = Online Store → Preferences → **Password** (the store password – Claude needs it to check the pages like a customer sees them)
...
- [ ] Settings → Domains → Connect existing domain → **adventlane.se** → follow the DNS steps → Set as primary
- [ ] Settings → Notifications → Sender email → **hello@adventlane.se** → Save → click the verification link in the inbox
...
## 10. Hand over
- [ ] Tell Claude Code: **"Store ready: AdventLane"** – it creates the pixel, builds Discord channels and imports reviews
- [ ] When Claude says the theme is ready: Online Store → Themes → **AdventLane – CRO v1** → **Publish**
