# Store Launch Checklist — HeimGuard (manual steps)
...
* STORE NAME: **HeimGuard**
* DOMAIN: **heimguard.se**
* STORE EMAIL: **hello@heimguard.se**
* FORWARD TO: **subscriptions@stonebite.org**
* OWNER (hand over to): **axelodhner.business@gmail.com**
* PRODUCT: **Övervakningskamera Trådlös – Dubbellins PTZ med AI-Spårning** (overvakningskameran)
...
- [ ] If Shopify asks for a store name, type **HeimGuard** – that removes a
...
- [ ] Online Store → Themes → **HeimGuard – CRO v1** → **Publish**
- [ ] Settings → General → Store name → **HeimGuard** → Save
...
- [ ] Log in to Loopia
- [ ] Buy **heimguard.se** – registrant must be the company, not you
- [ ] Domain → Email → Forwarding → create **hello@heimguard.se** → forward to **subscriptions@stonebite.org**
- [ ] Send a test email to **hello@heimguard.se** – confirm it arrives
- [ ] Shopify → Settings → Domains → Connect existing domain → **heimguard.se** → follow the DNS steps → **Set as primary**
- [ ] Shopify → Settings → Notifications → Sender email → **hello@heimguard.se** → Save → click the verification link in the inbox
...
- [ ] Judge.me → Settings → Import reviews → Import from apps → **Judge.me format** → upload the reviews file Claude gives you for **Övervakningskamera Trådlös – Dubbellins PTZ med AI-Spårning** → Import
...
- [ ] business.facebook.com → Settings → Pages → Add → Create a new Page: **HeimGuard**
...
- [ ] Same screen → the new page **HeimGuard** → **Add people** → pick the OWNER (the person whose Meta token Claude runs on) → tick **Manage Page** → **Assign**
...
- [ ] Discord → + → Create server: **HeimGuard — OPS**
...
- [ ] Write to Claude Code: **"Store ready: HeimGuard"** – it creates the pixel and builds the Discord channels
...
- [ ] WeTracked → paste the **pixel ID**: **1125401473242596**
- [ ] Events Manager → Data sources → **HeimGuard** → Settings → Conversions API → **Generate access token** → copy it
...
- [ ] Open **heimguard.se** on a phone as a customer: add to cart → checkout
...
- [ ] Write: **/ny-annonser hemvakten** + the Bäverbutiken product link
- [ ] The link is needed once per store — after that just **/ny-annonser hemvakten**
...
- [ ] The owner writes **"Launch: HeimGuard"** in that session – Claude checks the store is live and the pixel fires, then sets the campaigns ACTIVE
