...
- [ ] Now add the SAME four values a SECOND time, under this store's own names.
  ...
  `SHOPIFY_SHOP_HEMVAKTEN` = the same .myshopify.com address
  `SHOPIFY_CLIENT_ID_HEMVAKTEN` = the same Client ID
  `SHOPIFY_CLIENT_SECRET_HEMVAKTEN` = the same Client secret
  `SHOPIFY_STOREFRONT_PASSWORD_HEMVAKTEN` = the same store password
...
- [ ] Settings → Domains → Connect existing domain → **heimguard.se** → follow the DNS steps → Set as primary
- [ ] Settings → Notifications → Sender email → **hello@heimguard.se** → Save → click the verification link in the inbox
...
## 9. Discord
- [ ] Discord → + → Create server: **HeimGuard**
- [ ] Open the invite link Claude Code gives you → **Authorize** the bot

## 10. Hand over
- [ ] Tell Claude Code: **"Store ready: HeimGuard"** – it creates the pixel, builds Discord channels and imports reviews
- [ ] When Claude says the theme is ready: Online Store → Themes → **HeimGuard – CRO v1** → **Publish**
