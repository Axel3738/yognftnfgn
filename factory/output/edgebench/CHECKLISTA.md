# Store Launch Checklist — EdgeBench (manual steps)
...
* STORE NAME: **EdgeBench**
* DOMAIN: **edgebench.se**
* STORE EMAIL: **hello@edgebench.se**
* FORWARD TO: **subscriptions@stonebite.org**
* OWNER (hand over to): **axelodhner.business@gmail.com**
* PRODUCT: **Bälteslipmaskin Mini 3-i-1 – Slipband, Slipsten & Polerskiva** (balteslipmaskinen)
...
- [ ] If Shopify asks for a store name, type **EdgeBench** – that removes a
...
- [ ] Online Store → Themes → **EdgeBench – CRO v1** → **Publish**
- [ ] Settings → General → Store name → **EdgeBench** → Save
...
- [ ] Log in to Loopia
- [ ] Buy **edgebench.se** – registrant must be the company, not you
- [ ] Domain → Email → Forwarding → create **hello@edgebench.se** → forward to **subscriptions@stonebite.org**
- [ ] Send a test email to **hello@edgebench.se** – confirm it arrives
- [ ] Shopify → Settings → Domains → Connect existing domain → **edgebench.se** → follow the DNS steps → **Set as primary**
- [ ] Shopify → Settings → Notifications → Sender email → **hello@edgebench.se** → Save → click the verification link in the inbox
...
- [ ] Judge.me → Settings → Import reviews → Import from apps → **Judge.me format** → upload the reviews file Claude gives you for **Bälteslipmaskin Mini 3-i-1 – Slipband, Slipsten & Polerskiva** → Import
...
- [ ] business.facebook.com → Settings → Pages → Add → Create a new Page: **EdgeBench**
...
- [ ] Discord → + → Create server: **EdgeBench — OPS**
...
- [ ] Write to Claude Code: **"Store ready: EdgeBench"** – it creates the pixel and builds the Discord channels
...
- [ ] WeTracked → paste the **pixel ID**: **2005404160146799**
- [ ] Events Manager → Data sources → **EdgeBench** → Settings → Conversions API → **Generate access token** → copy it
...
- [ ] Open **edgebench.se** on a phone as a customer: add to cart → checkout
...
- [ ] Write: **/ny-annonser edgebench** + the Bäverbutiken product link
- [ ] The link is needed once per store — after that just **/ny-annonser edgebench**
...
- [ ] The owner writes **"Launch: EdgeBench"** in that session – Claude checks the store is live and the pixel fires, then sets the campaigns ACTIVE
