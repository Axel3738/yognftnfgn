# Store Launch Checklist — FjordCover (manual steps)
...
* STORE NAME: **FjordCover**
* DOMAIN: **fjordcover.se**
* STORE EMAIL: **hello@fjordcover.se**
* FORWARD TO: **subscriptions@stonebite.org**
* OWNER (hand over to): **axelodhner.business@gmail.com**
* PRODUCT: **Båtmotorskydd 420D – Heltäckande för Utombordare** (batmotorskyddet)
...
- [ ] If Shopify asks for a store name, type **FjordCover** – that removes a
...
- [ ] The app → **Configuration** → under **Access scopes** paste this exact line → **Save** → **Release** (top right).
  ...
  at step 0 and prints this same line (measured 2026-09-10, TackleBay: 0 of 16 scopes):
...
- [ ] Online Store → Themes → **FjordCover – CRO v1** → **Publish**
- [ ] Settings → General → Store name → **FjordCover** → Save
...
- [ ] Log in to Loopia
- [ ] Buy **fjordcover.se** – registrant must be the company, not you
- [ ] Domain → Email → Forwarding → create **hello@fjordcover.se** → forward to **subscriptions@stonebite.org**
- [ ] Send a test email to **hello@fjordcover.se** – confirm it arrives
- [ ] Shopify → Settings → Domains → Connect existing domain → **fjordcover.se** → follow the DNS steps → **Set as primary**
- [ ] Shopify → Settings → Notifications → Sender email → **hello@fjordcover.se** → Save → click the verification link in the inbox
...
- [ ] Judge.me → Settings → Import reviews → Import from apps → **Judge.me format** → upload the reviews file Claude gives you for **Båtmotorskydd 420D – Heltäckande för Utombordare** → Import
...
- [ ] business.facebook.com → Settings → Pages → Add → Create a new Page: **FjordCover**
...
- [ ] Discord → + → Create server: **FjordCover — OPS**
...
- [ ] Write to Claude Code: **"Store ready: FjordCover"** – it creates the pixel and builds the Discord channels
...
- [ ] WeTracked → paste the **pixel ID**: **1774648277117873**
- [ ] Events Manager → Data sources → **FjordCover** → Settings → Conversions API → **Generate access token** → copy it
...
- [ ] Open **fjordcover.se** on a phone as a customer: add to cart → checkout
...
- [ ] Write: **/ny-annonser fjordcover** + the Bäverbutiken product link
- [ ] The link is needed once per store — after that just **/ny-annonser fjordcover**
...
- [ ] The owner writes **"Launch: FjordCover"** in that session – Claude checks the store is live and the pixel fires, then sets the campaigns ACTIVE
