...
- [ ] Now add the SAME four values a SECOND time, under this store's own names.
  ...
  `SHOPIFY_SHOP_DRYTREK` = the same .myshopify.com address
  `SHOPIFY_CLIENT_ID_DRYTREK` = the same Client ID
  `SHOPIFY_CLIENT_SECRET_DRYTREK` = the same Client secret
  `SHOPIFY_STOREFRONT_PASSWORD_DRYTREK` = the same store password
