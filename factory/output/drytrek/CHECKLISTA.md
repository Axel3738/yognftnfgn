...
* STORE NAME: **DryTrek**
* DOMAIN: **drytrek.se**
* STORE EMAIL: **hello@drytrek.se**
* FORWARD TO: **subscriptions@stonebite.org**
* OWNER (hand over to): **axelodhner.business@gmail.com**
* PRODUCT: **Damasker Vandring – Håller Snö, Väta & Grus Ute** (damasker)
...
- [ ] This store's ID is **drytrek**. You need it twice: in the variable names below, and in the build command.
- [ ] Open the Claude session's **Environment** → ADD these 4. Do NOT touch the ones without an ID at the end — those belong to other stores:
  `SHOPIFY_SHOP_DRYTREK` = the store's .myshopify.com address
  `SHOPIFY_CLIENT_ID_DRYTREK` = the Client ID
  `SHOPIFY_CLIENT_SECRET_DRYTREK` = the Client secret
  `SHOPIFY_STOREFRONT_PASSWORD_DRYTREK` = Online Store → Preferences → **Password** (the store password – Claude needs it to check the pages like a customer sees them)
