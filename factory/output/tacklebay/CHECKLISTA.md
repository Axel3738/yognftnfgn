...
- [ ] If Shopify asks for a store name, type **TackleBay** – that removes a
...
- [ ] Shopify → Settings → Domains → Connect existing domain → **tacklebay.se** → follow the DNS steps → **Set as primary**
- [ ] Shopify → Settings → Notifications → Sender email → **hello@tacklebay.se** → Save → click the verification link in the inbox
...
- [ ] Same screen → the new page **TackleBay** → **Add people** → pick the OWNER (the person whose Meta token Claude runs on) → tick **Manage Page** → **Assign**
...
- [ ] Discord → + → Create server: **TackleBay — OPS**
...
- [ ] Write to Claude Code: **"Store ready: TackleBay"** – it creates the pixel and builds the Discord channels
...
- [ ] Open **tacklebay.se** on a phone as a customer: add to cart → checkout
...
- [ ] Write: **/ny-annonser tacklebay** + the Bäverbutiken product link
- [ ] The link is needed once per store — after that just **/ny-annonser tacklebay**
...
- [ ] The owner writes **"Launch: TackleBay"** in that session – Claude checks the store is live and the pixel fires, then sets the campaigns ACTIVE
