...
- [ ] This store's ID is **tacklebay**. You need it twice: in the variable names below, and in the build command.
- [ ] Open the Claude session's **Environment** → ADD these 4. Do NOT touch the ones without an ID at the end — those belong to other stores:
  `SHOPIFY_SHOP_TACKLEBAY` = the store's .myshopify.com address
  `SHOPIFY_CLIENT_ID_TACKLEBAY` = the Client ID
  `SHOPIFY_CLIENT_SECRET_TACKLEBAY` = the Client secret
  `SHOPIFY_STOREFRONT_PASSWORD_TACKLEBAY` = Online Store → Preferences → **Password** (the store password – Claude needs it to check the pages like a customer sees them)
...
- [ ] When Claude says the theme is ready: Online Store → Themes → **TackleBay v1** → **Publish**
