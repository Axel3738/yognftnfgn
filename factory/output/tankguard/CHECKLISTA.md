...
- [ ] If Shopify asks for a store name, type **TankGuard** – that removes a
...
- [ ] Online Store → Themes → **TankGuard – CRO v1** → **Publish**
...
- [ ] Shopify → Settings → Domains → Connect existing domain → **tankguard.se** → follow the DNS steps → **Set as primary**
- [ ] Shopify → Settings → Notifications → Sender email → **hello@tankguard.se** → Save → click the verification link in the inbox
...
- [ ] Same screen → the new page **TankGuard** → **Add people** → pick the OWNER (the person whose Meta token Claude runs on) → tick **Manage Page** → **Assign**
...
- [ ] Discord → + → Create server: **TankGuard — OPS**
...
- [ ] Write to Claude Code: **"Store ready: TankGuard"** – it creates the pixel and builds the Discord channels
...
- [ ] Open **tankguard.se** on a phone as a customer: add to cart → checkout
...
- [ ] The owner writes **"Launch: TankGuard"** in that session – Claude checks the store is live and the pixel fires, then sets the campaigns ACTIVE
