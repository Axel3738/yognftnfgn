...
- [ ] This store's ID is **tankguard**. You need it twice: in the variable names below, and in the build command.
- [ ] Open the Claude session's **Environment** → ADD these 4. Do NOT touch the ones without an ID at the end — those belong to other stores:
  `SHOPIFY_SHOP_TANKGUARD` = the store's .myshopify.com address
  `SHOPIFY_CLIENT_ID_TANKGUARD` = the Client ID
  `SHOPIFY_CLIENT_SECRET_TANKGUARD` = the Client secret
  `SHOPIFY_STOREFRONT_PASSWORD_TANKGUARD` = Online Store → Preferences → **Password** (the store password – Claude needs it to check the pages like a customer sees them)
