#!/usr/bin/env node
// trippelkoll-kampanj.mjs — steg 10 i /ny-annonser för TackleBay: läs
// TILLBAKA hela kampanjen ur Meta och jämför mot butikens konfig.
//
//   NODE_USE_ENV_PROXY=1 node factory/output/fiskespohallare-4-pack/trippelkoll-kampanj.mjs                # SE
//   NODE_USE_ENV_PROXY=1 node factory/output/fiskespohallare-4-pack/trippelkoll-kampanj.mjs --marknad NO   # NO
//   … --launchad   förväntar ACTIVE på alla tre nivåer (efter "Launch: TackleBay")
//
// Kontrollerar sida, pixel, budget/CBO, länk, geo, status på tre nivåer, och
// att varje annons i vågplanen (med: true) finns. Skriver inget i kontot.
...
const VANTAD = arg.includes('--launchad') ? 'ACTIVE' : 'PAUSED';
const plan = JSON.parse(readFileSync(join(HAR, M === 'NO' ? 'vagplan-no.json' : 'vagplan.json'), 'utf8'));
const ACT = 'act_915422744950975';
const PAGE = '1283919631474370';
const PIXEL = '1079980541064515';
const LINK = M === 'NO' ? 'https://tacklebay.se/nb/products/fiskespohallare-4-pack' : 'https://tacklebay.se/products/fiskespohallare-4-pack';
const BUDGET = '100000';
...
(k.status === VANTAD ? ok : fel).push(`kampanj status ${k.status}`);
...
for (const a of adsets) {
  ...
  (a.status === VANTAD ? ok : fel).push(`adset ${a.name}: status ${a.status}`);
  ...
  (geo === M ? ok : fel).push(`adset ${a.name}: geo ${geo}`);
...
for (const x of ads) {
  ...
  if (x.status !== VANTAD) { statusFel++; fel.push(`annons ${x.name}: status ${x.status}`); }
}
ok.push(`${ads.length} annonser uppe · länk rätt på ${ads.length - lankFel} · sida rätt på ${ads.length - sidFel} · ${VANTAD} på ${ads.length - statusFel}`);
