# Store Launch Checklist — CatCabin (manual steps)
...
* STORE NAME: **CatCabin**
* DOMAIN: **catcabin.se**
* STORE EMAIL: **hello@catcabin.se**
* FORWARD TO: **subscriptions@stonebite.org**
* OWNER (hand over to): **axelodhner.business@gmail.com**
* PRODUCT: **Isolerad Utekattkoja på Ben – Upphöjd Från Kall och Blöt Mark** (utekattkojan)
...
- [ ] If Shopify asks for a store name, type **CatCabin** – that removes a
...
- [ ] Online Store → Themes → **CatCabin – CRO v1** → **Publish**
- [ ] Settings → General → Store name → **CatCabin** → Save
...
- [ ] Log in to Loopia
- [ ] Buy **catcabin.se** – registrant must be the company, not you
- [ ] Domain → Email → Forwarding → create **hello@catcabin.se** → forward to **subscriptions@stonebite.org**
- [ ] Send a test email to **hello@catcabin.se** – confirm it arrives
- [ ] Shopify → Settings → Domains → Connect existing domain → **catcabin.se** → follow the DNS steps → **Set as primary**
- [ ] Shopify → Settings → Notifications → Sender email → **hello@catcabin.se** → Save → click the verification link in the inbox
...
- [ ] Judge.me → Settings → Import reviews → Import from apps → **Judge.me format** → upload the reviews file Claude gives you for **Isolerad Utekattkoja på Ben – Upphöjd Från Kall och Blöt Mark** → Import
...
- [ ] business.facebook.com → Settings → Pages → Add → Create a new Page: **CatCabin**
...
- [ ] Discord → + → Create server: **CatCabin — OPS**
...
- [ ] Write to Claude Code: **"Store ready: CatCabin"** – it creates the pixel and builds the Discord channels
...
- [ ] WeTracked → paste the **pixel ID**: **1101742182294878**
- [ ] Events Manager → Data sources → **CatCabin** → Settings → Conversions API → **Generate access token** → copy it
...
- [ ] Open **catcabin.se** on a phone as a customer: add to cart → checkout
...
- [ ] Write: **/ny-annonser catcabin** + the Bäverbutiken product link
- [ ] The link is needed once per store — after that just **/ny-annonser catcabin**
...
- [ ] The owner writes **"Launch: CatCabin"** in that session – Claude checks the store is live and the pixel fires, then sets the campaigns ACTIVE
