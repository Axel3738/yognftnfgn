...
3. **Om `AKUT`:** bygg briefen nu — hypotes, isolerad variabel, komplett brief enligt leveransformatet (engelska, Swedish/English-tabell, naming med nästa lediga AD-ID i MagiBorsten-kontot). **Copyn/voiceovern skrivs av en subagent med `model: "sonnet"`** (strategin stannar hos huvudmodellen). **Hard rule i briefen: annonsen nämner aldrig butikens namn** (Axels beslut 2026-09-18, `docs/copy-regler.md`) — annonserna speglas till OPS-butikerna och ska gå att återanvända utan att göras om. Lägg i Notion enligt `docs/os/NOTION-FORMAT.md` (item = annonsnamnet, Draft, tag `Video - Pending Approval`, brief i itemet), markera backlog-posten `[brief klar]`.
