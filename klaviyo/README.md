...
| Segment | ✅ 14 st: `SEG_samtycke` **2 890**, `SEG_uppvarmning_steg1` 175, `SEG_engagerade_60d` 175, `_90d` 176, `SEG_kopare` 2 596, `SEG_kopare_30d` 175, `SEG_flerkopare` 112, `SEG_ej_kopt` 294, `SEG_vinback_90d` 2 420, `SEG_oengagerade_180d` 0, kategorierna sushi 2 583 / donut 81 / pizza 10 / hamburgare 5 (mätt direkt efter skapandet). **+ `SEG_recension_kopare` `T9SCW8`** (2026-09-26: köpt någon gång AND inget köp senaste 16 d, **utan samtyckesgrupp**, `kopare: true`, aldrig kampanjpublik) — triggern för F08. **3 618 profiler** direkt efter skapandet (2026-09-26 06:40 CEST, klart bearbetat). |
| **F08 Recension → Judge.me** (Axels beslut B 2026-09-26 på frågan "vilka gamla köpare får recensionsmejlet?": **alla köpare som inte tackat nej**) | ✅ `FLOW_segment_recension_v1` **`XPvYvy`**, `draft`, mall `TPL_f08-recension-e1_v1` `Xh8cej` (flödets egen kopia `Vq54ns` — Klaviyo kopierar mallen in i flödet), tillbakaläst via API 2026-09-26: trigger segment `T9SCW8`, `profile_filter` = `kundundantag` (MFL 19 § andra stycket, samma som F04/F05/F07 — motorn släpper det igenom i ett segmenttriggat flöde bara när segmentet är märkt `kopare: true`, `ladda-upp.test.mjs`), ett mejl, återinträde aldrig. Bakgrund: produktsidan visade 8 recensioner av 3 776 köpare och 7 av 8 kom via Shop-appen (`products/matstrumpor/produktsida.md`) — recensionerna ska landa där sidan visar dem, alltså Judge.me (inte Trustpilot; `PROMPT-matstrumpor-recension.md` är ersatt). Knappen går till `…/products/sushi-strumpor#judgeme_product_reviews`, länkraden (nytt block `lankrad`) till pizza/hamburgare/donut; Judge.me verifierar köpet mot mejladressen (`review_verification_email_status: always` i widgetinställningarna). Sidfoten säger sanningen för icke-medlemmar (`sidfot_varfor` per mejl: "Du får det här mejlet för att du har handlat hos Matstrumpor."), sidhuvudets klubbrad är av (`utan_klubbrad`). Ingen belöning, inget krav på betyg, inga påminnelser. Copyn av en Sonnet-subagent mot copy-reglerna (noll tankstreck, inga priser, ingen leveranstid), tre-frågorstestet i flödesfilen. **Slås på på Axels ord när postadressen finns i kontot**; gamla köpare läggs in med Klaviyos **Add past profiles** (ikonen på triggerkortet i flödet, "relative to now") — det finns inte i API:t, så det är Axels klick; nya köpare kommer in av sig själva 16 dagar efter ordern. ⚠️ **F04/F05/F07 v3 bär fortfarande brandets sidfotsrad ("Du anmälde dig själv till Matstrumpor-klubben …") fast de går på kundundantaget till köpare som aldrig gått med** — rätta med v4 (ny flödesversion, `stada.mjs` på v3) innan de slås på. |
...
## Lärdomar 2026-09-26 (Matstrumpor, recensionsflödet)

- **Kundundantaget i ett segmenttriggat flöde:** motorns spärr "kundundantag bara i
  Placed/Fulfilled Order-flöden" var rätt tanke men fel gräns — det som bär undantaget
  är kundrelationen, inte triggern. Ett segment som BARA består av köpare (`kopare: true`
  i `segment.mjs`, utan samtyckesgrupp, `kampanjOk: false`) får därför trigga ett
  kundundantagsflöde. Alla andra segment stoppas som förut, och en kampanj kan aldrig gå
  till ett sådant segment (`kravSamtycke`).
- **Sidfoten och sidhuvudet måste följa mottagaren, inte brandet.** Brandets rad "Du
  anmälde dig själv till Matstrumpor-klubben" och eyebrown "Klubbpost. Inte för alla." är
  osanna för en köpare som aldrig gått med. Därför `sidfot_varfor` och `utan_klubbrad`
  per mejl. F04/F05/F07 v3 (utkast) har felet kvar och ska få v4 innan de slås på.
- **Recensionerna ska landa där sidan visar dem.** Produktsidan visar Judge.me, så ett
  Trustpilot-flöde hade byggt bevis på fel ställe. Judge.me:s webbformulär verifierar
  köpet mot mejladressen, så länken kan vara sidans egen `#judgeme_product_reviews`.
- **Klaviyo har inget API för "Add past profiles".** Ett segmenttriggat flöde når bara
  dem som kommer in i segmentet efter att flödet slagits på; de 3 700 gamla köparna
  kräver ett klick på triggerkortet i Klaviyos gränssnitt. Bygg aldrig ett "skicka till
  alla gamla köpare" som kampanj för att slippa klicket — järnregel 2.
